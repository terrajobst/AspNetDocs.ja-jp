...
---
<a name="1160--2016-12-01"></a>1.16.0 / 2016 年 12 月 1 日
==================

## <a name="additional"></a>追加
  * cifES アルゴリズムと nieES アルゴリズムの改善。 #1826 の終了

## <a name="build"></a>ビルド
  * NPM パッケージにおける縮小バージョンの追加
  * 開発依存関係の最新バージョンへの昇格

## <a name="core"></a>コア
  * ボタンの種類での入力のバインドの追加。 #1891 の終了
  * jquery3 のサポート。 #1866 の終了
  * jQuery の別名 'expr[":"]' の 'expr.pseudos' への変更

## <a name="localisation"></a>ローカリゼーション
  * ウルドゥ語翻訳の追加。 #1873 の終了。

## <a name="localization"></a>ローカリゼーション
  * az 翻訳の間違ったファイル拡張子の修正。 #1890 の終了。
  * pt-BR での不足していた翻訳の追加 (#1897 の終了)
  * アラビア語ファイルにおける入力ミスの修正。

## <a name="tests"></a>テスト
  * QUnit の 2.0 へのアップグレード。

## <a name="umd"></a>UMD
  * CommonJS のサポートの向上。

<a name="1151--2016-07-22"></a>1.15.1/2016 年 7 月 22 日
==================

## <a name="additional"></a>追加
  * 複数のMIME タイプの検証の修正
  * IBAN には少なくとも 5 文字が必要 (#1797 の終了、#1674 の修正)
  * notEqualTo jQuery リファレンスの修正

## <a name="core"></a>コア
  * #1805 の失敗したテストの追加
  * 3 つ以上のフィールドでのグループ検証の修正
  * #1644 と #1657 で導入された回帰の修正 (#1800 の終了)
  * 浮動小数点数を正しく処理するためのステップ検証の更新
  * 非フォーム要素で $.fn.rules() を呼び出すときのエラーの修正
  * 検証コントロールのスコープにおける `errorPlacement` の呼び出し
  * 1 つの入力を検証するためのイベントで例外が発生するフォームのコンテンツ編集可能な要素での問題の修正

## <a name="demo"></a>デモ
  * ブートストラップと Semantic-UI のデモへのリンクの index.html への追加
  * `.validateDelegate()` の代わりの `.on()` の使用

## <a name="localization"></a>ローカリゼーション
  * アゼルバイジャン語の追加

## <a name="tests"></a>テスト
  * PR #1760 の回帰の単体テストの追加

<a name="1150--2016-02-24"></a>1.15.0 / 2016 年 2 月 24 日
==================
...
## <a name="core"></a>コア
  * `resetForm` は要素から `valid` クラスを削除する必要もあります。
  * リモート ルールを使用する際にフィールドが既に強調表示されていた場合の、強調表示の解除。
  * `equalTo` ルールで `blur` イベントを 1 回だけバインドする
  * 空の jquery セットで .rules() を呼び出す際のエラーの修正。
  * 入力グループでのエラー メッセージの処理の修正。
  * `groups` 設定を使用する場合の `showLabel` における TypeError の修正
  * メソッド名をリモートに渡す方法の追加
  * 次のフィールドが既に入力されている場合、検証がトリガーに失敗します (#1508 の修正)
  * 数値と桁のルールに対する必須ルールの優先
  * エラーが非表示だが入力エラー クラスが削除されない
  * リモート検証による間違ったエラー メッセージの使用
  * リモート検証でのフィールドの強調表示の修正。
  * 複数の選択要素に対する `:filled` セレクターの修正。
  * jQuery.validator.methods に対するドキュメント参照の追加
  * メッセージ処理の `formatAndAdd` から `defaultMessage` への移動
  * ErrorList には適切なエラーのみが含まれる必要があります
  * "C:\fakepath\" を含まないファイル名の抽出
  * HTML5 のステップ属性のサポート。 #1295 の修正
  * 未処理の要求での "pending" クラスのサポートの追加
  * ノーマライザーの追加 (#1602)
  * `creditcard` メソッドの分割
  * 正規表現で使用するための errorID のエスケープ (aria-describedby をビルドしない)
  * Sizzle エラーの発生を回避するための名前の単一引用符のエスケープ
  * API 呼び出しをスキップするために検証フィールドの値を使用する代わりに、要求のシリアル化されたデータ オブジェクトを使用する
  * contentEditable タグのサポートの追加

## <a name="additional"></a>追加
  * BIC: 位置の 2 つ目の場所では 1 から 9 の数字が許可されます
  * 受け入れメソッドの正規表現は適切にエスケープする必要があります。
  * BIC の大文字と小文字を区別しないチェック
  * 無効な組み合わせを除外するための postalCodeCA の修正
  * postalCodeCA メソッドの厳密化

## <a name="localization"></a>ローカリゼーション
  * マケドニア語のローカリゼーションの追加。
  * ポーランド語で不足していたパターン メッセージの追加 (adamwojtkiewicz)
  * 最小/最大メッセージのペルシャ語翻訳の修正。
  * messages_sk.js の更新
  * マレー語翻訳の更新
  * 追加メソッドからのメッセージの追加
  * pt_BR 翻訳の改善と 'cifES' キーでの入力ミスの修正。
