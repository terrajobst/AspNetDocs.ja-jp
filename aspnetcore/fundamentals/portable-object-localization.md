...
**注:** Orchard Core では、Microsoft 製品はありません。 したがって、Microsoft はこの機能をサポートしていません。
...
- `msgid_plural`:無変換の複数形の文字列。
- `msgstr[0]`:0 の場合の翻訳された文字列。
- `msgstr[N]`:翻訳後の文字列の大文字の n ください。
...
**注:** 実際のシナリオでは、数を表すために、変数は使用されます。 ここでは、非常に特殊なケースを表すために、3 つの異なる値を使用して同じコードを繰り返しています。
