...
`X-Forwarded-For` および `X-Forwarded-Proto` ヘッダーを転送する場合は、<xref:host-and-deploy/linux-nginx#configure-nginx> を参照してください。 詳細については、次を参照してください[NGINX:。転送ヘッダーを使用して](https://www.nginx.com/resources/wiki/start/topics/examples/forwarded/)します。

## <a name="apache-configuration"></a>Apache の構成

`X-Forwarded-For` 自動的に追加されます (を参照してください[Apache モジュール mod_proxy:リバース プロキシの要求ヘッダー](https://httpd.apache.org/docs/2.4/mod/mod_proxy.html#x-headers))。 `X-Forwarded-Proto` ヘッダーを転送する方法については、<xref:host-and-deploy/linux-apache#configure-apache> を参照してください。
