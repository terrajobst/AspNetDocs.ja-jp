...
> [!NOTE]
> timeout、domain、および、path など、いくつかのフォーム認証設定は、結果として得られるフォーム認証チケットの Cookie の詳細を指定します。 Cookie、そのしくみ、およびそのさまざまなプロパティの詳細については、[Cookie チュートリアル](http://www.quirksmode.org/js/cookies.html) をご覧ください。
