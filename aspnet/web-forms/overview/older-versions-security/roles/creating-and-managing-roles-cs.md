...
`DeleteRole`場合は、メソッドに例外はスローも*roleName*は`null`または空の文字列場合*roleName*コンマが含まれています。 場合*roleName* 、システムに存在しません`DeleteRole`サイレント モードでは、例外を発生させずに失敗します。
