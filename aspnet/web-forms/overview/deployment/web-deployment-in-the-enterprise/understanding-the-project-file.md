...
プロパティ値を取得する形式を使用して、 *$ (propertyname)* します。 たとえばの値を取得するため、 **ServerName**入力プロパティ。
...
- プロパティ値を使用する入力 **$(***PropertyName***)** ここで、 *PropertyName*の名前を指定します、**プロパティ**要素の名前または、パラメーター。
- 項目を使用する入力 **@(***ItemName***)** ここで、 *ItemName*の名前を指定します、**項目**要素。
