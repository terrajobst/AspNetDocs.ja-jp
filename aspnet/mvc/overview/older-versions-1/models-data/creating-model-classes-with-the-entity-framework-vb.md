...
\_Db フィールドは、映画データベース テーブルからレコードを取得する Index() 操作内で使用します。 式\_db します。MovieSet では、映画データベース テーブルからすべてのレコードを表します。 ToList() メソッドを使用して、ムービーのムービー オブジェクトのジェネリック コレクションに変換します。(ビデオ) の一覧です。
