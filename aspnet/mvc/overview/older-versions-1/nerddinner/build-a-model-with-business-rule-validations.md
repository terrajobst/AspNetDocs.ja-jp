...
## <a name="nerddinner-step-3-building-the-model"></a>NerdDinner 手順 3:モデルを構築します。
...
ASP.NET MVC フレームワークには、任意のデータ アクセス テクノロジを使用して、開発者は、さまざまななど、モデルを実装するために豊富な .NET データのオプションから選択できます。LINQ to エンティティ、LINQ to SQL、NHibernate、LLBLGen Pro、SubSonic、WilsonORM、または単なる生 ADO.NET Datareader またはデータセットです。
...
次のコードを使用するときは必ず DinnerRepository クラスを実装しましたできます。
