...
- *Mvc4Async*:このチュートリアルで使用するコードを含む、ASP.NET MVC 4 プロジェクトです。 Web API の呼び出し、 **WebAPIpgw**サービス。
...
- *WebAppAsync*:別のチュートリアルで使用する ASP.NET Web フォーム プロジェクト。
...
サンプル アプリケーションを選択する、*キャンセル トークンのデモ*呼び出しのリンク、`GizmosCancelAsync`メソッドの非同期呼び出しのキャンセルを示しています。
