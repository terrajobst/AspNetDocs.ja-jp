...
2. *dataValueField*:プロパティの名前、 **IEnumerable**キーの値を含む一覧。 上記の例で`GenreId`と`ArtistId`します。
3. *dataTextField*:プロパティの名前、 **IEnumerable**一覧を表示する情報を格納します。 アーティストとジャンルのテーブルの両方で、`name`フィールドを使用します。
