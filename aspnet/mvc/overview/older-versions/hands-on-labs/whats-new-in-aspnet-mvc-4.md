...
このドキュメントの付録を参照することができます、Visual Studio のコード スニペットとその使用方法を学習するに慣れていない場合&quot;[付録 a:コード スニペットを使用して](#AppendixA)&quot;します。
...
4. キーを押して**F5**ソリューションを実行し、更新されたテンプレートを参照してください。 次の機能を確認することができます。
    ...
    *jQuery の検証*
    ...
    > [!NOTE]
    > サイトから登録済みのアカウントを使用して (既定で無効になっている) google などの別の認証サービスを使用してログインできる別の方法として 2 番目のセクションにログインできる最初のセクションのセクションでは、2 つのログを確認します。
...
2. 開く、**パッケージ マネージャー コンソール**をクリックして、**ツール** > **NuGet パッケージ マネージャー** > **パッケージ マネージャー コンソール**メニュー オプション。
...
このタスクでは、モバイル デバイスでより優れた外観に適応させるコンテンツを含む index ビューのモバイル バージョンを作成します。
...
3. 開く、  **\_ViewSwitcher.cshtml**部分ビュー。
    ...
    **GetOverriddenBrowser**メソッドを返します、 **HttpBrowserCapabilitiesBase**要求に設定されているユーザー エージェントに対応するインスタンス (実際またはオーバーライド) します。 この値を使用して、プロパティを取得します。 **IsMobileDevice**します。
...
4. 開く、 **ViewSwitcherController.cs**クラス、**コント ローラー**フォルダー。 チェック アウトする SwitchView アクションは ViewSwitcher コンポーネントでリンクによって呼び出され、HttpContext の新しいメソッドに注目してください。
    ...
    - **HttpContext.ClearOverriddenBrowser()** メソッドは、現在の要求でオーバーライドされたユーザー エージェントを削除します。
...
## <a name="appendix-a-using-code-snippets"></a>付録 a:コード スニペットを使用
