...
この新しい StoreController は既に"Index"メソッドを使用しています。 この"Index"メソッドを使用して、ミュージック ストア内のすべてのジャンルを一覧表示された、リスト ページを実装します。 シナリオを実装する、2 つの他の処理するために、StoreController する 2 つの追加メソッドも追加します。参照と詳細情報。
