...
3. <span data-ttu-id="0d6b6-270">クリックして**コンピューティング**、 **web サイト**し**カスタム作成**です。</span><span class="sxs-lookup"><span data-stu-id="0d6b6-270">Click **Compute**, **Website** and then **Custom Create**.</span></span>
    ...
    <span data-ttu-id="0d6b6-272">*カスタム作成を使用して新しい web アプリを作成します。*</span><span class="sxs-lookup"><span data-stu-id="0d6b6-272">*Creating a new web app using Custom Create*</span></span>
...
4. <span data-ttu-id="0d6b6-348">移動して、**ダッシュ ボード**ページし、をクリックして**ステージングされた発行を有効にする**下、**概要**セクション。</span><span class="sxs-lookup"><span data-stu-id="0d6b6-348">Go to the **Dashboard** page and click **Enable staged publishing** under the **quick glance** section.</span></span>
    ...
    <span data-ttu-id="0d6b6-350">*ステージングされた発行を有効にします。*</span><span class="sxs-lookup"><span data-stu-id="0d6b6-350">*Enabling staged publishing*</span></span>
...
1. <span data-ttu-id="0d6b6-401">管理ポータルに戻るし、ステージング サイト スロットを選択します。</span><span class="sxs-lookup"><span data-stu-id="0d6b6-401">Go back to the management portal and select the staging site slot.</span></span> <span data-ttu-id="0d6b6-402">クリックして**スワップ**コマンド バーにします。</span><span class="sxs-lookup"><span data-stu-id="0d6b6-402">Click **Swap** in the command bar.</span></span>

    ![運用環境にスワップします。](maintainable-azure-websites-managing-change-and-scale/_static/image44.png)

    <span data-ttu-id="0d6b6-404">*運用環境にスワップします。*</span><span class="sxs-lookup"><span data-stu-id="0d6b6-404">*Swap to production*</span></span>
2. <span data-ttu-id="0d6b6-405">クリックして**はい**スワップ操作を続行する確認のダイアログ ボックスでします。</span><span class="sxs-lookup"><span data-stu-id="0d6b6-405">Click **Yes** in the confirmation dialog box to proceed with the swap operation.</span></span> <span data-ttu-id="0d6b6-406">Azure は、ステージング サイトのコンテンツを含む実稼働サイトのコンテンツをすぐにスワップされます。</span><span class="sxs-lookup"><span data-stu-id="0d6b6-406">Azure will immediately swap the content of the production site with the content of the staging site.</span></span>

    > [!NOTE]
    > <span data-ttu-id="0d6b6-407">(例: 接続文字列の上書き、ハンドラー マッピングなど) の production バージョンには、ステージングされたバージョンからのいくつかの設定が自動的にコピーされますが、(例: DNS エンドポイント、SSL のバインドなど)、その他の設定は変更されません。</span><span class="sxs-lookup"><span data-stu-id="0d6b6-407">Some settings from the staged version will automatically be copied to the production version (e.g. connection string overrides, handler mappings, etc.) but other settings will not change (e.g. DNS endpoints, SSL bindings, etc.).</span></span>

    ![スワップ操作を確認します。](maintainable-azure-websites-managing-change-and-scale/_static/image45.png)

    <span data-ttu-id="0d6b6-409">*スワップ操作を確認します。*</span><span class="sxs-lookup"><span data-stu-id="0d6b6-409">*Confirming swap operation*</span></span>
3. <span data-ttu-id="0d6b6-410">スワップが完了すると、運用スロットを選択し、をクリックして**参照**コマンド バーを実稼働サイトを開きます。</span><span class="sxs-lookup"><span data-stu-id="0d6b6-410">Once the swap is complete, select the production slot and click **Browse** in the command bar to open the production site.</span></span> <span data-ttu-id="0d6b6-411">アドレス バーの URL に注意してください。</span><span class="sxs-lookup"><span data-stu-id="0d6b6-411">Notice the URL in the address bar.</span></span>

    > [!NOTE]
    > <span data-ttu-id="0d6b6-412">キャッシュをクリアするブラウザーを更新する必要があります。</span><span class="sxs-lookup"><span data-stu-id="0d6b6-412">You might need to refresh your browser to clear cache.</span></span> <span data-ttu-id="0d6b6-413">Internet Explorer で、これを行うキーを押して**CTRL + R**します。</span><span class="sxs-lookup"><span data-stu-id="0d6b6-413">In Internet Explorer, you can do this by pressing **CTRL+R**.</span></span>

    ![運用環境で実行されている web アプリ](maintainable-azure-websites-managing-change-and-scale/_static/image46.png)
4. <span data-ttu-id="0d6b6-415">**GitBash**コンソールで、運用スロットを対象に、ローカルの Git リポジトリのリモート URL を更新します。</span><span class="sxs-lookup"><span data-stu-id="0d6b6-415">In the **GitBash** console, update the remote URL for the local Git repository to target the production slot.</span></span> <span data-ttu-id="0d6b6-416">これを行うには、プレース ホルダーをデプロイ ユーザー名と、web アプリの名前に置き換えて、次のコマンドを実行します。</span><span class="sxs-lookup"><span data-stu-id="0d6b6-416">To do this, run the following command replacing the placeholders with your deployment username and the name of your web app.</span></span>

    > [!NOTE]
    > <span data-ttu-id="0d6b6-417">次の演習では、ステージング ラボの簡潔さのためだけではなく、運用サイトへ変更をプッシュします。</span><span class="sxs-lookup"><span data-stu-id="0d6b6-417">In the following exercises, you will push changes to the production site instead of staging just for the simplicity of the lab.</span></span> <span data-ttu-id="0d6b6-418">実際のシナリオで、運用環境に昇格する前にステージング環境の変更を確認することをお勧めします。</span><span class="sxs-lookup"><span data-stu-id="0d6b6-418">In a real-world scenario, it is recommended to verify the changes in the staging environment before promoting to production.</span></span>

    [!code-console[Main](maintainable-azure-websites-managing-change-and-scale/samples/sample13.cmd)]

<a id="Exercise3"></a>
### <a name="exercise-3-performing-deployment-rollback-in-production"></a><span data-ttu-id="0d6b6-419">手順 3:運用環境でデプロイのロールバックを実行します。</span><span class="sxs-lookup"><span data-stu-id="0d6b6-419">Exercise 3: Performing Deployment Rollback in Production</span></span>

<span data-ttu-id="0d6b6-420">場所がないステージングや運用などの間でのホット スワップを実行するステージング スロットを使用する場合のシナリオがある**Free**または**Shared**モード。</span><span class="sxs-lookup"><span data-stu-id="0d6b6-420">There are scenarios where you do not have a staging slot to perform hot swap between staging and production, for example, if you are working with **Free** or **Shared** mode.</span></span> <span data-ttu-id="0d6b6-421">そのようなシナリオでする必要がありますアプリケーションをテストするテスト環境で – ローカルまたはリモート サイトで – 運用環境にデプロイする前にします。</span><span class="sxs-lookup"><span data-stu-id="0d6b6-421">In those scenarios, you should test your application in a testing environment –either locally or in a remote site– before deploying to production.</span></span> <span data-ttu-id="0d6b6-422">ただし、実稼働サイトに、テスト フェーズ中に検出されない問題が発生することが可能です。</span><span class="sxs-lookup"><span data-stu-id="0d6b6-422">However, it is possible that an issue not detected during the testing phase may arise in the production site.</span></span> <span data-ttu-id="0d6b6-423">この場合、可能な限り早く前とより安定したバージョンのアプリケーションに切り替える簡単にするためのメカニズムを理解しておくことができます。</span><span class="sxs-lookup"><span data-stu-id="0d6b6-423">In this case, it is important to have a mechanism to easily switch to a previous and more stable version of the application as quickly as possible.</span></span>

<span data-ttu-id="0d6b6-424">**Azure App Service**、ソース管理からの継続的なデプロイは、考えられるこのおかげ、**再デプロイ**管理ポータルで使用できるアクション。</span><span class="sxs-lookup"><span data-stu-id="0d6b6-424">In **Azure App Service**, continuous deployment from source control makes this possible thanks to the **redeploy** action available in the management portal.</span></span> <span data-ttu-id="0d6b6-425">Azure では、リポジトリにプッシュされたコミットに関連付けられている展開を追跡し、いつでも、以前の展開のいずれかを使用してアプリケーションを再デプロイするためのオプションを提供します。</span><span class="sxs-lookup"><span data-stu-id="0d6b6-425">Azure keeps track of the deployments associated with the commits pushed to the repository and provides an option to redeploy your application using any of your previous deployments, at any time.</span></span>

<span data-ttu-id="0d6b6-426">この演習では、コードに変更を実行して、**ギーク Quiz**意図的に挿入するアプリケーション、*バグ*します。</span><span class="sxs-lookup"><span data-stu-id="0d6b6-426">In this exercise you will perform a change to the code in the **Geek Quiz** application that intentionally injects a *bug*.</span></span> <span data-ttu-id="0d6b6-427">ときに、運用環境にアプリケーションを展開するし、以前の状態に戻るに再デプロイ機能がかかります。</span><span class="sxs-lookup"><span data-stu-id="0d6b6-427">You will deploy the application to production to see the error, and then you will take advantage of the redeploy feature to go back to the previous state.</span></span>

<a id="Ex3Task1"></a>
#### <a name="task-1--updating-the-geek-quiz-application"></a><span data-ttu-id="0d6b6-428">タスク 1 – おたくクイズ アプリケーションの更新</span><span class="sxs-lookup"><span data-stu-id="0d6b6-428">Task 1 – Updating the Geek Quiz Application</span></span>

<span data-ttu-id="0d6b6-429">このタスクでは、小型のコードのリファクタリングが、 **TriviaController**クラスの新しいメソッドに、データベースから選択したクイズ オプションを取得するロジックの一部を抽出します。</span><span class="sxs-lookup"><span data-stu-id="0d6b6-429">In this task, you will refactor a small piece of code of the **TriviaController** class to extract part of the logic that retrieves the selected quiz option from the database into a new method.</span></span>

1. <span data-ttu-id="0d6b6-430">Visual Studio のインスタンスに切り替え、 **GeekQuiz**前の演習からソリューション。</span><span class="sxs-lookup"><span data-stu-id="0d6b6-430">Switch to the Visual Studio instance with the **GeekQuiz** solution from the previous exercise.</span></span>
2. <span data-ttu-id="0d6b6-431">**ソリューション エクスプ ローラー**、オープン、 **TriviaController.cs**ファイル内で、**コント ローラー**フォルダー。</span><span class="sxs-lookup"><span data-stu-id="0d6b6-431">In **Solution Explorer**, open the **TriviaController.cs** file inside the **Controllers** folder.</span></span>
3. <span data-ttu-id="0d6b6-432">検索、 **StoreAsync**メソッドと、コードは次の図で強調表示を選択します。</span><span class="sxs-lookup"><span data-stu-id="0d6b6-432">Locate the **StoreAsync** method and select the code highlighted in the following figure.</span></span>

    ![コードを選択します。](maintainable-azure-websites-managing-change-and-scale/_static/image47.png)

    <span data-ttu-id="0d6b6-434">*コードを選択します。*</span><span class="sxs-lookup"><span data-stu-id="0d6b6-434">*Selecting the code*</span></span>
4. <span data-ttu-id="0d6b6-435">選択したコードを右クリックし、展開、**リファクタリング**メニュー選択し、**メソッドの抽出しています.**.</span><span class="sxs-lookup"><span data-stu-id="0d6b6-435">Right-click the selected code, expand the **Refactor** menu and select **Extract Method...**.</span></span>

    ![新しいメソッドとして、コードを抽出](maintainable-azure-websites-managing-change-and-scale/_static/image48.png)

    <span data-ttu-id="0d6b6-437">*メソッドの抽出を選択します。*</span><span class="sxs-lookup"><span data-stu-id="0d6b6-437">*Selecting Extract Method*</span></span>
5. <span data-ttu-id="0d6b6-438">**メソッドの抽出**ダイアログ ボックスで、新しいメソッドの名前*MatchesOption*  をクリック**OK**します。</span><span class="sxs-lookup"><span data-stu-id="0d6b6-438">In the **Extract Method** dialog box, name the new method *MatchesOption* and click **OK**.</span></span>

    ![メソッド名を指定します。](maintainable-azure-websites-managing-change-and-scale/_static/image49.png)

    <span data-ttu-id="0d6b6-440">*抽出されたメソッドの名前を指定します。*</span><span class="sxs-lookup"><span data-stu-id="0d6b6-440">*Specifying the name for the extracted method*</span></span>
6. <span data-ttu-id="0d6b6-441">選択したコードに抽出し、 **MatchesOption**メソッド。</span><span class="sxs-lookup"><span data-stu-id="0d6b6-441">The selected code is then extracted into the **MatchesOption** method.</span></span> <span data-ttu-id="0d6b6-442">結果として得られるコードは、次のスニペットに示します。</span><span class="sxs-lookup"><span data-stu-id="0d6b6-442">The resulting code is shown in the following snippet.</span></span>

    [!code-csharp[Main](maintainable-azure-websites-managing-change-and-scale/samples/sample14.cs)]
7. <span data-ttu-id="0d6b6-443">キーを押して**CTRL + S**変更を保存します。</span><span class="sxs-lookup"><span data-stu-id="0d6b6-443">Press **CTRL + S** to save the changes.</span></span>

<a id="Ex3Task2"></a>
#### <a name="task-2--redeploying-the-geek-quiz-application"></a><span data-ttu-id="0d6b6-444">タスク 2 – おたくクイズのアプリケーションを再デプロイします。</span><span class="sxs-lookup"><span data-stu-id="0d6b6-444">Task 2 – Redeploying the Geek Quiz Application</span></span>

<span data-ttu-id="0d6b6-445">運用環境への新しい配置をトリガーすると、リポジトリに、前のタスクで加えた変更をプッシュします。</span><span class="sxs-lookup"><span data-stu-id="0d6b6-445">You will now push the changes you made in the previous task to the repository, which will trigger a new deployment to the production environment.</span></span> <span data-ttu-id="0d6b6-446">次を使用して、問題のトラブルシューティングは、 **F12 開発ツール**、Internet Explorer によって提供され、Azure 管理ポータルから、以前のデプロイにロールバックを実行します。</span><span class="sxs-lookup"><span data-stu-id="0d6b6-446">Then, you will troubleshot an issue using the **F12 development tools** provided by Internet Explorer, and then perform a rollback to the previous deployment from the Azure management portal.</span></span>

1. <span data-ttu-id="0d6b6-447">新しく開きます**Git Bash**更新されたアプリケーションを Azure App Service をデプロイするコンソール。</span><span class="sxs-lookup"><span data-stu-id="0d6b6-447">Open a new **Git Bash** console to deploy the updated application to Azure App Service.</span></span>
2. <span data-ttu-id="0d6b6-448">Azure に変更をプッシュするには、次のコマンドを実行します。</span><span class="sxs-lookup"><span data-stu-id="0d6b6-448">Execute the following commands to push the changes to Azure.</span></span> <span data-ttu-id="0d6b6-449">更新プログラム、 *[YOUR APPLICATION PATH]* へのパスのプレース ホルダー、 **GeekQuiz**ソリューション。</span><span class="sxs-lookup"><span data-stu-id="0d6b6-449">Update the *[YOUR-APPLICATION-PATH]* placeholder with the path to the **GeekQuiz** solution.</span></span> <span data-ttu-id="0d6b6-450">デプロイ パスワードを求めるメッセージが表示されます。</span><span class="sxs-lookup"><span data-stu-id="0d6b6-450">You will be prompted for your deployment password.</span></span>

    [!code-console[Main](maintainable-azure-websites-managing-change-and-scale/samples/sample15.cmd)]

    ![リファクタリング後のコードを Azure にプッシュ](maintainable-azure-websites-managing-change-and-scale/_static/image50.png)

    <span data-ttu-id="0d6b6-452">*リファクタリング後のコードを Azure にプッシュ*</span><span class="sxs-lookup"><span data-stu-id="0d6b6-452">*Pushing refactored code to Azure*</span></span>
3. <span data-ttu-id="0d6b6-453">Internet Explorer を開き、web アプリに移動します (例: `http://<your-web-site>.azurewebsites.net`)。</span><span class="sxs-lookup"><span data-stu-id="0d6b6-453">Open Internet Explorer and navigate to your web app (e.g. `http://<your-web-site>.azurewebsites.net`).</span></span> <span data-ttu-id="0d6b6-454">以前に作成した資格情報を使用してをログインします。</span><span class="sxs-lookup"><span data-stu-id="0d6b6-454">Log in using the previously created credentials.</span></span>
4. <span data-ttu-id="0d6b6-455">キーを押して**F12**開発ツールを起動するには、次のように選択します。、**ネットワーク** タブで  をクリックし、**再生**記録を開始するボタンをクリックします。</span><span class="sxs-lookup"><span data-stu-id="0d6b6-455">Press **F12** to launch the development tools, select the **Network** tab and click the **Play** button to start recording.</span></span>

    <span data-ttu-id="0d6b6-456">![ネットワークの記録を開始](maintainable-azure-websites-managing-change-and-scale/_static/image51.png "ネットワークの記録を開始")</span><span class="sxs-lookup"><span data-stu-id="0d6b6-456">![Starting network recording](maintainable-azure-websites-managing-change-and-scale/_static/image51.png "Starting network recording")</span></span>

    <span data-ttu-id="0d6b6-457">*ネットワークの記録を開始*</span><span class="sxs-lookup"><span data-stu-id="0d6b6-457">*Starting network recording*</span></span>
5. <span data-ttu-id="0d6b6-458">クイズの任意のオプションを選択します。</span><span class="sxs-lookup"><span data-stu-id="0d6b6-458">Select any option of the quiz.</span></span> <span data-ttu-id="0d6b6-459">何も起こらないことが表示されます。</span><span class="sxs-lookup"><span data-stu-id="0d6b6-459">You will see that nothing happens.</span></span>
6. <span data-ttu-id="0d6b6-460">**F12**ウィンドウで、POST HTTP 要求に対応するエントリを示しています、HTTP **500**結果。</span><span class="sxs-lookup"><span data-stu-id="0d6b6-460">In the **F12** window, the entry corresponding to the POST HTTP request shows an HTTP **500** result.</span></span>

    ![HTTP 500 エラー](maintainable-azure-websites-managing-change-and-scale/_static/image52.png)

    <span data-ttu-id="0d6b6-462">*HTTP 500 エラー*</span><span class="sxs-lookup"><span data-stu-id="0d6b6-462">*HTTP 500 error*</span></span>
7. <span data-ttu-id="0d6b6-463">選択、**コンソール**タブ。エラーが原因の詳細を記録します。</span><span class="sxs-lookup"><span data-stu-id="0d6b6-463">Select the **Console** tab. An error is logged with the details of the cause.</span></span>

    ![ログに記録されたエラー](maintainable-azure-websites-managing-change-and-scale/_static/image53.png)

    <span data-ttu-id="0d6b6-465">*ログに記録されたエラー*</span><span class="sxs-lookup"><span data-stu-id="0d6b6-465">*Logged error*</span></span>
8. <span data-ttu-id="0d6b6-466">エラーの詳細部分を見つけます。</span><span class="sxs-lookup"><span data-stu-id="0d6b6-466">Locate the details part of the error.</span></span> <span data-ttu-id="0d6b6-467">明らかに、リファクタリング前の手順でコミットするコードによってこのエラーが発生します。</span><span class="sxs-lookup"><span data-stu-id="0d6b6-467">Clearly, this error is caused by the code refactoring you committed in the previous steps.</span></span>

    <span data-ttu-id="0d6b6-468">`Details: LINQ to Entities does not recognize the method 'Boolean MatchesOption ...`.</span><span class="sxs-lookup"><span data-stu-id="0d6b6-468">`Details: LINQ to Entities does not recognize the method 'Boolean MatchesOption ...`.</span></span>
9. <span data-ttu-id="0d6b6-469">ブラウザーを閉じないでください。</span><span class="sxs-lookup"><span data-stu-id="0d6b6-469">Do not close the browser.</span></span>
10. <span data-ttu-id="0d6b6-470">新しいブラウザー インスタンスでは、に移動、 [Azure 管理ポータル](https://manage.windowsazure.com)サブスクリプションに関連付けられた Microsoft アカウントを使用してサインインします。</span><span class="sxs-lookup"><span data-stu-id="0d6b6-470">In a new browser instance, navigate to the [Azure management portal](https://manage.windowsazure.com) and sign in using the Microsoft account associated with your subscription.</span></span>
11. <span data-ttu-id="0d6b6-471">選択**Websites**手順 2 で作成した web アプリをクリックします。</span><span class="sxs-lookup"><span data-stu-id="0d6b6-471">Select **Websites** and click the web app you created in Exercise 2.</span></span>
12. <span data-ttu-id="0d6b6-472">移動し、**展開**ページ。</span><span class="sxs-lookup"><span data-stu-id="0d6b6-472">Navigate to the **Deployments** page.</span></span> <span data-ttu-id="0d6b6-473">デプロイ履歴で、実行されるすべてのコミットが表示されていることに注意してください。</span><span class="sxs-lookup"><span data-stu-id="0d6b6-473">Notice that all the commits performed are listed in the deployment history.</span></span>

    ![既存の展開の一覧](maintainable-azure-websites-managing-change-and-scale/_static/image54.png)

    <span data-ttu-id="0d6b6-475">*既存の展開の一覧*</span><span class="sxs-lookup"><span data-stu-id="0d6b6-475">*List of existing deployments*</span></span>
13. <span data-ttu-id="0d6b6-476">以前のコミットを選択し、クリックして**再デプロイ**コマンド バーにします。</span><span class="sxs-lookup"><span data-stu-id="0d6b6-476">Select the previous commit and click **Redeploy** on the command bar.</span></span>

    ![以前のコミットを再デプロイします。](maintainable-azure-websites-managing-change-and-scale/_static/image55.png)

    <span data-ttu-id="0d6b6-478">*以前のコミットを再デプロイします。*</span><span class="sxs-lookup"><span data-stu-id="0d6b6-478">*Redeploying the previous commit*</span></span>
14. <span data-ttu-id="0d6b6-479">確認するメッセージが表示されたら、クリックして**はい**します。</span><span class="sxs-lookup"><span data-stu-id="0d6b6-479">When prompted to confirm, click **Yes**.</span></span>

    ![再展開を確認します。](maintainable-azure-websites-managing-change-and-scale/_static/image56.png)
15. <span data-ttu-id="0d6b6-481">デプロイが完了したらは、web アプリとキーを押して、ブラウザー インスタンスに切り替える**ctrl キーを押しながら f5 キーを押して**します。</span><span class="sxs-lookup"><span data-stu-id="0d6b6-481">When the deployment completes, switch back to the browser instance with your web app and press **CTRL + F5**.</span></span>
16. <span data-ttu-id="0d6b6-482">任意のオプションをクリックします。</span><span class="sxs-lookup"><span data-stu-id="0d6b6-482">Click any of the options.</span></span> <span data-ttu-id="0d6b6-483">フリップ アニメーション処理を実行し、結果 (*修正または正しくない*) が表示されます。</span><span class="sxs-lookup"><span data-stu-id="0d6b6-483">The flip animation will now take place and the result (*correct/incorrect*) will be displayed.</span></span>
17. <span data-ttu-id="0d6b6-484">(省略可能)切り替えて、 **Git Bash**コンソールし、以前のコミットに戻すには、次のコマンドを実行します。</span><span class="sxs-lookup"><span data-stu-id="0d6b6-484">(Optional) Switch to the **Git Bash** console and execute the following commands to revert to the previous commit.</span></span>

    > [!NOTE]
    > <span data-ttu-id="0d6b6-485">これらのコマンドは、Git リポジトリに正しくないコミットに加えられたすべての変更を元に戻すための新しいコミットを作成します。</span><span class="sxs-lookup"><span data-stu-id="0d6b6-485">These commands create a new commit that undoes all changes in the Git repository that were made in the bad commit.</span></span> <span data-ttu-id="0d6b6-486">Azure は、新しいコミットを使用して、アプリケーションとし、再デプロイします。</span><span class="sxs-lookup"><span data-stu-id="0d6b6-486">Azure will then redeploy the application using the new commit.</span></span>

    [!code-console[Main](maintainable-azure-websites-managing-change-and-scale/samples/sample16.cmd)]

<a id="Exercise4"></a>
### <a name="exercise-4-scaling-using-azure-storage"></a><span data-ttu-id="0d6b6-487">手順 4:Azure Storage を使用したスケーリング</span><span class="sxs-lookup"><span data-stu-id="0d6b6-487">Exercise 4: Scaling Using Azure Storage</span></span>

<span data-ttu-id="0d6b6-488">**Blob**は大量の非構造化テキストまたはバイナリ データなど、ビデオ、オーディオ、イメージを格納する最も簡単な方法です。</span><span class="sxs-lookup"><span data-stu-id="0d6b6-488">**Blobs** are the simplest way to store large amounts of unstructured text or binary data such as video, audio and images.</span></span> <span data-ttu-id="0d6b6-489">記憶域をアプリケーションの静的コンテンツを移動するは、画像またはドキュメントをブラウザーに直接配信することによって、アプリケーションをスケールするのに役立ちます。</span><span class="sxs-lookup"><span data-stu-id="0d6b6-489">Moving the static content of your application to Storage, helps to scale your application by serving images or documents directly to the browser.</span></span>

<span data-ttu-id="0d6b6-490">この演習では、Blob コンテナーに、アプリケーションの静的コンテンツを移動します。</span><span class="sxs-lookup"><span data-stu-id="0d6b6-490">In this exercise, you will move the static content of your application to a Blob container.</span></span> <span data-ttu-id="0d6b6-491">アプリケーションを追加するを構成し、 **ASP.NET URL の書き換え規則**で、 **Web.config**コンテンツを Blob コンテナーにリダイレクトします。</span><span class="sxs-lookup"><span data-stu-id="0d6b6-491">Then you will configure your application to add an **ASP.NET URL rewrite rule** in the **Web.config** to redirect your content to the Blob container.</span></span>

<a id="Ex4Task1"></a>
#### <a name="task-1--creating-an-azure-storage-account"></a><span data-ttu-id="0d6b6-492">タスク 1 – Azure ストレージ アカウントの作成</span><span class="sxs-lookup"><span data-stu-id="0d6b6-492">Task 1 – Creating an Azure Storage Account</span></span>

<span data-ttu-id="0d6b6-493">このタスクでは、管理ポータルを使用して、新しいストレージ アカウントを作成する方法を学びます。</span><span class="sxs-lookup"><span data-stu-id="0d6b6-493">In this task you will learn how to create a new storage account using the management portal.</span></span>

1. <span data-ttu-id="0d6b6-494">移動し、 [Azure 管理ポータル](https://manage.windowsazure.com)サブスクリプションに関連付けられた Microsoft アカウントを使用してサインインします。</span><span class="sxs-lookup"><span data-stu-id="0d6b6-494">Navigate to the [Azure management portal](https://manage.windowsazure.com) and sign in using the Microsoft account associated with your subscription.</span></span>
2. <span data-ttu-id="0d6b6-495">選択**新しい |Data Services |記憶域 |簡易作成**新しいストレージ アカウントの作成を開始します。</span><span class="sxs-lookup"><span data-stu-id="0d6b6-495">Select **New | Data Services | Storage | Quick Create** to start creating a new storage account.</span></span> <span data-ttu-id="0d6b6-496">クリックし、アカウントの一意の名前を入力、**リージョン**一覧から。</span><span class="sxs-lookup"><span data-stu-id="0d6b6-496">Enter a unique name for the account and select a **Region** from the list.</span></span> <span data-ttu-id="0d6b6-497">クリックして**ストレージ アカウントの作成**を続行します。</span><span class="sxs-lookup"><span data-stu-id="0d6b6-497">Click **Create Storage Account** to continue.</span></span>

    <span data-ttu-id="0d6b6-498">![新しいストレージ アカウントを作成する](maintainable-azure-websites-managing-change-and-scale/_static/image57.png "新しいストレージ アカウントの作成")</span><span class="sxs-lookup"><span data-stu-id="0d6b6-498">![Creating a new Storage Account](maintainable-azure-websites-managing-change-and-scale/_static/image57.png "Creating a new Storage Account")</span></span>

    <span data-ttu-id="0d6b6-499">*新しいストレージ アカウントの作成*</span><span class="sxs-lookup"><span data-stu-id="0d6b6-499">*Creating a new storage account*</span></span>
3. <span data-ttu-id="0d6b6-500">**ストレージ**セクションで、新しいストレージ アカウントの状態に変わるまで待機*オンライン*次の手順を続行するためです。</span><span class="sxs-lookup"><span data-stu-id="0d6b6-500">In the **Storage** section, wait until the status of the new storage account changes to *Online* in order to continue with the following step.</span></span>

    <span data-ttu-id="0d6b6-501">![作成したストレージ アカウント](maintainable-azure-websites-managing-change-and-scale/_static/image58.png "ストレージ アカウントの作成")</span><span class="sxs-lookup"><span data-stu-id="0d6b6-501">![Storage Account created](maintainable-azure-websites-managing-change-and-scale/_static/image58.png "Storage Account created")</span></span>

    <span data-ttu-id="0d6b6-502">*ストレージ アカウントの作成*</span><span class="sxs-lookup"><span data-stu-id="0d6b6-502">*Storage Account created*</span></span>
4. <span data-ttu-id="0d6b6-503">ストレージ アカウントの名前をクリックし、**ダッシュ ボード**ページの上部にあるリンクです。</span><span class="sxs-lookup"><span data-stu-id="0d6b6-503">Click on the storage account name and then click the **Dashboard** link at the top of the page.</span></span> <span data-ttu-id="0d6b6-504">**ダッシュ ボード**ページによって、アカウントと、アプリケーション内で使用できるサービス エンドポイントの状態に関する情報を提供します。</span><span class="sxs-lookup"><span data-stu-id="0d6b6-504">The **Dashboard** page provides you with information about the status of the account and the service endpoints that can be used within your applications.</span></span>

    <span data-ttu-id="0d6b6-505">![ストレージ アカウント ダッシュ ボードを表示する](maintainable-azure-websites-managing-change-and-scale/_static/image59.png "ストレージ アカウント ダッシュ ボードを表示します。")</span><span class="sxs-lookup"><span data-stu-id="0d6b6-505">![Displaying the Storage Account Dashboard](maintainable-azure-websites-managing-change-and-scale/_static/image59.png "Displaying the Storage Account Dashboard")</span></span>

    <span data-ttu-id="0d6b6-506">*ストレージ アカウント ダッシュ ボードを表示します。*</span><span class="sxs-lookup"><span data-stu-id="0d6b6-506">*Displaying the Storage Account Dashboard*</span></span>
5. <span data-ttu-id="0d6b6-507">をクリックして、**アクセス キーの管理**ナビゲーション バーのボタンをクリックします。</span><span class="sxs-lookup"><span data-stu-id="0d6b6-507">Click the **Manage Access Keys** button in the navigation bar.</span></span>

    <span data-ttu-id="0d6b6-508">![アクセス キーの管理 ボタン](maintainable-azure-websites-managing-change-and-scale/_static/image60.png "アクセス キーの管理 ボタン")</span><span class="sxs-lookup"><span data-stu-id="0d6b6-508">![Manage Access Keys button](maintainable-azure-websites-managing-change-and-scale/_static/image60.png "Manage Access Keys button")</span></span>

    <span data-ttu-id="0d6b6-509">*[アクセス キー] ボタンを管理します。*</span><span class="sxs-lookup"><span data-stu-id="0d6b6-509">*Manage Access Keys button*</span></span>
6. <span data-ttu-id="0d6b6-510">**アクセス キーの管理**ダイアログ ボックスで、コピー、**ストレージ アカウント名**と**プライマリ アクセス キー**に次の手順で必要になります。</span><span class="sxs-lookup"><span data-stu-id="0d6b6-510">In the **Manage Access Keys** dialog box, copy the **Storage Account Name** and **Primary Access Key** as you will need them in the following exercise.</span></span> <span data-ttu-id="0d6b6-511">次に、ダイアログ ボックスを閉じます。</span><span class="sxs-lookup"><span data-stu-id="0d6b6-511">Then, close the dialog box.</span></span>

    <span data-ttu-id="0d6b6-512">![アクセス キーの管理 ダイアログ ボックス](maintainable-azure-websites-managing-change-and-scale/_static/image61.png "アクセス キーの管理 ダイアログ ボックス")</span><span class="sxs-lookup"><span data-stu-id="0d6b6-512">![Manage Access Key dialog box](maintainable-azure-websites-managing-change-and-scale/_static/image61.png "Manage Access Key dialog box")</span></span>

    <span data-ttu-id="0d6b6-513">*管理アクセス キー ダイアログ ボックス*</span><span class="sxs-lookup"><span data-stu-id="0d6b6-513">*Manage Access Key dialog box*</span></span>

<a id="Ex4Task2"></a>
#### <a name="task-2--uploading-an-asset-to-azure-blob-storage"></a><span data-ttu-id="0d6b6-514">タスク 2 – Azure Blob Storage への資産のアップロード</span><span class="sxs-lookup"><span data-stu-id="0d6b6-514">Task 2 – Uploading an Asset to Azure Blob Storage</span></span>

<span data-ttu-id="0d6b6-515">このタスクでは、ストレージ アカウントに接続する Visual Studio から、サーバー エクスプ ローラー ウィンドウを使用します。</span><span class="sxs-lookup"><span data-stu-id="0d6b6-515">In this task, you will use the Server Explorer window from Visual Studio to connect to your storage account.</span></span> <span data-ttu-id="0d6b6-516">Blob コンテナーを作成し、ギーク Quiz ロゴのファイルをコンテナーにアップロードされます。</span><span class="sxs-lookup"><span data-stu-id="0d6b6-516">You will then create a blob container and upload a file with the Geek Quiz logo to the container.</span></span>

1. <span data-ttu-id="0d6b6-517">Visual Studio のインスタンスに切り替え、 **GeekQuiz**前の演習からソリューション。</span><span class="sxs-lookup"><span data-stu-id="0d6b6-517">Switch to the Visual Studio instance with the **GeekQuiz** solution from the previous exercise.</span></span>
2. <span data-ttu-id="0d6b6-518">メニュー バーから選択**ビュー**  をクリックし、**サーバー エクスプ ローラー**します。</span><span class="sxs-lookup"><span data-stu-id="0d6b6-518">From the menu bar, select **View** and then click **Server Explorer**.</span></span>
3. <span data-ttu-id="0d6b6-519">**サーバー エクスプ ローラー**を右クリックし、 **Azure**ノード**を Azure に接続しています.**.サブスクリプションに関連付けられている Microsoft アカウントを使用してサインインします。</span><span class="sxs-lookup"><span data-stu-id="0d6b6-519">In **Server Explorer**, right-click the **Azure** node and select **Connect to Azure...**. Sign in using the Microsoft account associated with your subscription.</span></span>

    ![Windows Azure への接続します。](maintainable-azure-websites-managing-change-and-scale/_static/image62.png)

    <span data-ttu-id="0d6b6-521">*Azure への接続します。*</span><span class="sxs-lookup"><span data-stu-id="0d6b6-521">*Connect to Azure*</span></span>
4. <span data-ttu-id="0d6b6-522">展開、 **Azure**ノードを右クリックし**ストレージ**選択と**外部ストレージのアタッチしています.**.</span><span class="sxs-lookup"><span data-stu-id="0d6b6-522">Expand the **Azure** node, right-click **Storage** and select **Attach External Storage...**.</span></span>
5. <span data-ttu-id="0d6b6-523">**新しいストレージ アカウントの追加** ダイアログ ボックスに、入力、**アカウント名**と**アカウント キー** 、前のタスクとクリックで取得した**OK**.</span><span class="sxs-lookup"><span data-stu-id="0d6b6-523">In the **Add New Storage Account** dialog box, enter the **Account name** and **Account key** you obtained in the previous task and click **OK**.</span></span>

    ![新しいストレージ アカウント ダイアログ ボックスを追加します。](maintainable-azure-websites-managing-change-and-scale/_static/image63.png)

    <span data-ttu-id="0d6b6-525">*新しいストレージ アカウント ダイアログ ボックスを追加します。*</span><span class="sxs-lookup"><span data-stu-id="0d6b6-525">*Add New Storage Account dialog box*</span></span>
6. <span data-ttu-id="0d6b6-526">ストレージ アカウントは、下に表示する必要があります、**ストレージ**ノード。</span><span class="sxs-lookup"><span data-stu-id="0d6b6-526">Your storage account should appear under the **Storage** node.</span></span> <span data-ttu-id="0d6b6-527">ストレージ アカウントを展開し、右クリックして**Blob**選択**Blob コンテナーの作成.**.</span><span class="sxs-lookup"><span data-stu-id="0d6b6-527">Expand your storage account, right-click **Blobs** and select **Create Blob Container...**.</span></span>

    <span data-ttu-id="0d6b6-528">![Blob コンテナーを作成する](maintainable-azure-websites-managing-change-and-scale/_static/image64.png "Blob コンテナーを作成します。")</span><span class="sxs-lookup"><span data-stu-id="0d6b6-528">![Create Blob Container](maintainable-azure-websites-managing-change-and-scale/_static/image64.png "Create Blob Container")</span></span>

    <span data-ttu-id="0d6b6-529">*Blob コンテナーを作成します。*</span><span class="sxs-lookup"><span data-stu-id="0d6b6-529">*Create Blob Container*</span></span>
7. <span data-ttu-id="0d6b6-530">**Blob コンテナーの作成** ダイアログ ボックスで、blob コンテナーの名前を入力し、をクリックして**OK**します。</span><span class="sxs-lookup"><span data-stu-id="0d6b6-530">In the **Create Blob Container** dialog box, enter a name for the blob container and click **OK**.</span></span>

    <span data-ttu-id="0d6b6-531">![Blob コンテナーの作成 ダイアログ ボックス](maintainable-azure-websites-managing-change-and-scale/_static/image65.png "Blob コンテナーの作成 ダイアログ ボックス")</span><span class="sxs-lookup"><span data-stu-id="0d6b6-531">![Create Blob Container dialog box](maintainable-azure-websites-managing-change-and-scale/_static/image65.png "Create Blob Container dialog box")</span></span>

    <span data-ttu-id="0d6b6-532">*Blob コンテナー ダイアログ ボックスを作成します。*</span><span class="sxs-lookup"><span data-stu-id="0d6b6-532">*Create Blob Container dialog box*</span></span>
8. <span data-ttu-id="0d6b6-533">新しい blob コンテナーに追加する必要があります、 **Blob**ノード。</span><span class="sxs-lookup"><span data-stu-id="0d6b6-533">The new blob container should be added to the **Blobs** node.</span></span> <span data-ttu-id="0d6b6-534">コンテナーを公開して、コンテナーのアクセス許可を変更します。</span><span class="sxs-lookup"><span data-stu-id="0d6b6-534">Change the access permissions in the container to make the container public.</span></span> <span data-ttu-id="0d6b6-535">これを行うを右クリックし、**イメージ**コンテナーと選択**プロパティ**します。</span><span class="sxs-lookup"><span data-stu-id="0d6b6-535">To do this, right-click the **images** container and select **Properties**.</span></span>

    <span data-ttu-id="0d6b6-536">![コンテナーのプロパティをイメージ](maintainable-azure-websites-managing-change-and-scale/_static/image66.png "イメージのコンテナーのプロパティ")</span><span class="sxs-lookup"><span data-stu-id="0d6b6-536">![images container properties](maintainable-azure-websites-managing-change-and-scale/_static/image66.png "images container properties")</span></span>

    <span data-ttu-id="0d6b6-537">*コンテナー イメージのプロパティ*</span><span class="sxs-lookup"><span data-stu-id="0d6b6-537">*Images container properties*</span></span>
9. <span data-ttu-id="0d6b6-538">**プロパティ**ウィンドウで、設定、**パブリック読み取りアクセス**に**コンテナー**します。</span><span class="sxs-lookup"><span data-stu-id="0d6b6-538">In the **Properties** window, set the **Public Read Access** to **Container**.</span></span>

    <span data-ttu-id="0d6b6-539">![パブリック読み取りアクセスのプロパティを変更する](maintainable-azure-websites-managing-change-and-scale/_static/image67.png "パブリック読み取りアクセスのプロパティを変更します。")</span><span class="sxs-lookup"><span data-stu-id="0d6b6-539">![Changing public read access property](maintainable-azure-websites-managing-change-and-scale/_static/image67.png "Changing public read access property")</span></span>

    <span data-ttu-id="0d6b6-540">*パブリック読み取りアクセスのプロパティを変更します。*</span><span class="sxs-lookup"><span data-stu-id="0d6b6-540">*Changing public read access property*</span></span>
10. <span data-ttu-id="0d6b6-541">パブリック アクセス プロパティを変更するには、をクリックする場合は、確認を求められたら**はい**します。</span><span class="sxs-lookup"><span data-stu-id="0d6b6-541">When prompted if you are sure you want to change the public access property, click **Yes**.</span></span>

    <span data-ttu-id="0d6b6-542">![Microsoft Visual Studio 警告](maintainable-azure-websites-managing-change-and-scale/_static/image68.png "Microsoft Visual Studio 警告")</span><span class="sxs-lookup"><span data-stu-id="0d6b6-542">![Microsoft Visual Studio warning](maintainable-azure-websites-managing-change-and-scale/_static/image68.png "Microsoft Visual Studio warning")</span></span>

    <span data-ttu-id="0d6b6-543">*Microsoft Visual Studio 警告*</span><span class="sxs-lookup"><span data-stu-id="0d6b6-543">*Microsoft Visual Studio warning*</span></span>
11. <span data-ttu-id="0d6b6-544">**サーバー エクスプ ローラー**でを右クリックし、**イメージ**blob コンテナーと選択**Blob コンテナーの表示**します。</span><span class="sxs-lookup"><span data-stu-id="0d6b6-544">In **Server Explorer**, right-click in the **images** blob container and select **View Blob Container**.</span></span>

    <span data-ttu-id="0d6b6-545">![Blob コンテナーを表示](maintainable-azure-websites-managing-change-and-scale/_static/image69.png "Blob コンテナーの表示")</span><span class="sxs-lookup"><span data-stu-id="0d6b6-545">![View Blob Container](maintainable-azure-websites-managing-change-and-scale/_static/image69.png "View Blob Container")</span></span>

    <span data-ttu-id="0d6b6-546">*Blob コンテナーの表示*</span><span class="sxs-lookup"><span data-stu-id="0d6b6-546">*View Blob Container*</span></span>
12. <span data-ttu-id="0d6b6-547">イメージ コンテナーが新しいウィンドウで開く必要があり、エントリがないの凡例を表示する必要があります。</span><span class="sxs-lookup"><span data-stu-id="0d6b6-547">The images container should open in a new window and a legend with no entries should be shown.</span></span> <span data-ttu-id="0d6b6-548">をクリックして、**アップロード**アイコン ファイルを blob コンテナーにアップロードします。</span><span class="sxs-lookup"><span data-stu-id="0d6b6-548">Click the **upload** icon to upload a file to the blob container.</span></span>

    <span data-ttu-id="0d6b6-549">![エントリがないイメージ コンテナー](maintainable-azure-websites-managing-change-and-scale/_static/image70.png "エントリがないイメージ コンテナー")</span><span class="sxs-lookup"><span data-stu-id="0d6b6-549">![Images container with no entries](maintainable-azure-websites-managing-change-and-scale/_static/image70.png "Images container with no entries")</span></span>

    <span data-ttu-id="0d6b6-550">*エントリがないイメージ コンテナー*</span><span class="sxs-lookup"><span data-stu-id="0d6b6-550">*Images container with no entries*</span></span>
13. <span data-ttu-id="0d6b6-551">**Blob のアップロード** ダイアログ ボックスに移動、**資産**ラボのフォルダー。</span><span class="sxs-lookup"><span data-stu-id="0d6b6-551">In the **Upload Blob** dialog box, navigate to the **Assets** folder of the lab.</span></span> <span data-ttu-id="0d6b6-552">選択、**ロゴ big.png**ファイルし、クリックして**オープン**。</span><span class="sxs-lookup"><span data-stu-id="0d6b6-552">Select the **logo-big.png** file and click **Open**.</span></span>
14. <span data-ttu-id="0d6b6-553">ファイルがアップロードされるまで待ちます。</span><span class="sxs-lookup"><span data-stu-id="0d6b6-553">Wait until the file is uploaded.</span></span> <span data-ttu-id="0d6b6-554">アップロードが完了したら、ファイルがイメージのコンテナーに表示されます。</span><span class="sxs-lookup"><span data-stu-id="0d6b6-554">When the upload completes, the file should be listed in the images container.</span></span> <span data-ttu-id="0d6b6-555">ファイルのエントリを右クリックして**URL のコピー**します。</span><span class="sxs-lookup"><span data-stu-id="0d6b6-555">Right-click the file entry and select **Copy URL**.</span></span>

    <span data-ttu-id="0d6b6-556">![Blob の URL をコピー](maintainable-azure-websites-managing-change-and-scale/_static/image71.png "blob ファイルの URL をコピー")</span><span class="sxs-lookup"><span data-stu-id="0d6b6-556">![Copy blob URL](maintainable-azure-websites-managing-change-and-scale/_static/image71.png "Copy blob file URL")</span></span>

    <span data-ttu-id="0d6b6-557">*Blob の URL をコピーします。*</span><span class="sxs-lookup"><span data-stu-id="0d6b6-557">*Copy blob URL*</span></span>
15. <span data-ttu-id="0d6b6-558">Internet Explorer を開き、URL を貼り付けます。</span><span class="sxs-lookup"><span data-stu-id="0d6b6-558">Open Internet Explorer and paste the URL.</span></span> <span data-ttu-id="0d6b6-559">次の図は、ブラウザーに表示する必要があります。</span><span class="sxs-lookup"><span data-stu-id="0d6b6-559">The following image should be shown in the browser.</span></span>

    <span data-ttu-id="0d6b6-560">![Windows の Blob ストレージからイメージをロゴ big.png](maintainable-azure-websites-managing-change-and-scale/_static/image72.png "ストレージからロゴ big.png イメージ")</span><span class="sxs-lookup"><span data-stu-id="0d6b6-560">![logo-big.png image from Windows Blob Storage](maintainable-azure-websites-managing-change-and-scale/_static/image72.png "logo-big.png image from storage")</span></span>

    <span data-ttu-id="0d6b6-561">*logo-big.png image from Azure Blob Storage*</span><span class="sxs-lookup"><span data-stu-id="0d6b6-561">*logo-big.png image from Azure Blob Storage*</span></span>

<a id="Ex4Task3"></a>
#### <a name="task-3--updating-the-solution-to-consume-static-content-from-azure-blob-storage"></a><span data-ttu-id="0d6b6-562">タスク 3 – Azure Blob Storage から静的コンテンツを使用するソリューションを更新しています</span><span class="sxs-lookup"><span data-stu-id="0d6b6-562">Task 3 – Updating the Solution to Consume Static Content from Azure Blob Storage</span></span>

<span data-ttu-id="0d6b6-563">このタスクでは、構成、 **GeekQuiz**ソリューション、イメージを使用する Azure Blob Storage にアップロード (web アプリにあるイメージ) ではなくで ASP.NET URL 書き換え規則を追加することで、 **web.config**ファイル。</span><span class="sxs-lookup"><span data-stu-id="0d6b6-563">In this task, you will configure the **GeekQuiz** solution to consume the image uploaded to Azure Blob Storage (instead of the image located in the web app) by adding an ASP.NET URL rewrite rule in the **web.config** file.</span></span>

1. <span data-ttu-id="0d6b6-564">Visual Studio で開く、 **Web.config**ファイル内で、 **GeekQuiz**プロジェクトし、検索、 **&lt;system.webServer&gt;** 要素。</span><span class="sxs-lookup"><span data-stu-id="0d6b6-564">In Visual Studio, open the **Web.config** file inside the **GeekQuiz** project and locate the **&lt;system.webServer&gt;** element.</span></span>
2. <span data-ttu-id="0d6b6-565">URL 書き換え規則が、プレース ホルダーをストレージ アカウント名で更新を追加するには、次のコードを追加します。</span><span class="sxs-lookup"><span data-stu-id="0d6b6-565">Add the following code to add an URL rewrite rule, updating the placeholder with your storage account name.</span></span>

    <span data-ttu-id="0d6b6-566">(コード スニペット - *WebSitesInProduction - Ex4 - UrlRewriteRule*)</span><span class="sxs-lookup"><span data-stu-id="0d6b6-566">(Code Snippet - *WebSitesInProduction - Ex4 - UrlRewriteRule*)</span></span>

    [!code-xml[Main](maintainable-azure-websites-managing-change-and-scale/samples/sample17.xml)]

    > [!NOTE]
    > <span data-ttu-id="0d6b6-567">URL リライトは、受信 Web 要求をインターセプトし、別のリソースへの要求のリダイレクトのプロセスです。</span><span class="sxs-lookup"><span data-stu-id="0d6b6-567">URL rewriting is the process of intercepting an incoming Web request and redirecting the request to a different resource.</span></span> <span data-ttu-id="0d6b6-568">URL 書き換えルールでは、書き換えエンジンを要求をリダイレクトする必要がある場合と、リダイレクト先にするように指示します。</span><span class="sxs-lookup"><span data-stu-id="0d6b6-568">The URL rewriting rules tells the rewriting engine when a request needs to be redirected, and where should they be redirected.</span></span> <span data-ttu-id="0d6b6-569">書き換えルールは 2 つの文字列で構成されます: 要求された URL で検索するパターン (通常は、正規表現を使用して) 場合に使用すると、パターンを置換する文字列が見つかったとします。</span><span class="sxs-lookup"><span data-stu-id="0d6b6-569">A rewriting rule is composed of two strings: the pattern to look for in the requested URL (usually, using regular expressions), and the string to replace the pattern with, if found.</span></span> <span data-ttu-id="0d6b6-570">詳細については、次を参照してください。 [ASP.NET における URL リライト](https://msdn.microsoft.com/library/ms972974.aspx)します。</span><span class="sxs-lookup"><span data-stu-id="0d6b6-570">For more information, see [URL Rewriting in ASP.NET](https://msdn.microsoft.com/library/ms972974.aspx).</span></span>
3. <span data-ttu-id="0d6b6-571">キーを押して**CTRL + S**変更を保存します。</span><span class="sxs-lookup"><span data-stu-id="0d6b6-571">Press **CTRL + S** to save the changes.</span></span>
4. <span data-ttu-id="0d6b6-572">新しく開きます**Git Bash**更新されたアプリケーションを Azure App Service をデプロイするコンソール。</span><span class="sxs-lookup"><span data-stu-id="0d6b6-572">Open a new **Git Bash** console to deploy the updated application to Azure App Service.</span></span>
5. <span data-ttu-id="0d6b6-573">Azure に変更をプッシュするには、次のコマンドを実行します。</span><span class="sxs-lookup"><span data-stu-id="0d6b6-573">Execute the following commands to push the changes to Azure.</span></span> <span data-ttu-id="0d6b6-574">更新プログラム、 *[YOUR APPLICATION PATH]* へのパスのプレース ホルダー、 **GeekQuiz**ソリューション。</span><span class="sxs-lookup"><span data-stu-id="0d6b6-574">Update the *[YOUR-APPLICATION-PATH]* placeholder with the path to the **GeekQuiz** solution.</span></span> <span data-ttu-id="0d6b6-575">デプロイ パスワードを求めるメッセージが表示されます。</span><span class="sxs-lookup"><span data-stu-id="0d6b6-575">You will be prompted for your deployment password.</span></span>

    [!code-console[Main](maintainable-azure-websites-managing-change-and-scale/samples/sample18.cmd)]

    ![Azure への更新プログラムの展開](maintainable-azure-websites-managing-change-and-scale/_static/image73.png)

    <span data-ttu-id="0d6b6-577">*Azure への更新プログラムの展開*</span><span class="sxs-lookup"><span data-stu-id="0d6b6-577">*Deploying update to Azure*</span></span>

<a id="Ex4Task4"></a>
#### <a name="task-4--verification"></a><span data-ttu-id="0d6b6-578">タスク 4 – 検証</span><span class="sxs-lookup"><span data-stu-id="0d6b6-578">Task 4 – Verification</span></span>

<span data-ttu-id="0d6b6-579">このタスクでは、使用**Internet Explorer**を参照する、**ギーク Quiz**でホストされているイメージにアプリケーションと、URL の書き換え規則がのイメージが機能し、チェックをリダイレクト**Azure Blob記憶域**します。</span><span class="sxs-lookup"><span data-stu-id="0d6b6-579">In this task you will use **Internet Explorer** to browse the **Geek Quiz** application and check that the URL rewrite rule for images works and you are redirected to the image hosted on **Azure Blob Storage**.</span></span>

1. <span data-ttu-id="0d6b6-580">Internet Explorer を開き、web アプリに移動します (例: `http://<your-web-site>.azurewebsites.net`)。</span><span class="sxs-lookup"><span data-stu-id="0d6b6-580">Open Internet Explorer and navigate to your web app (e.g. `http://<your-web-site>.azurewebsites.net`).</span></span> <span data-ttu-id="0d6b6-581">以前に作成した資格情報を使用してをログインします。</span><span class="sxs-lookup"><span data-stu-id="0d6b6-581">Log in using the previously created credentials.</span></span>

    <span data-ttu-id="0d6b6-582">![イメージとギーク Quiz web アプリを表示](maintainable-azure-websites-managing-change-and-scale/_static/image74.png "イメージ ギーク Quiz web アプリを表示")</span><span class="sxs-lookup"><span data-stu-id="0d6b6-582">![Showing the Geek Quiz web app with the image](maintainable-azure-websites-managing-change-and-scale/_static/image74.png "Showing the Geek Quiz web app with the image")</span></span>

    <span data-ttu-id="0d6b6-583">*イメージとギーク Quiz web アプリを表示*</span><span class="sxs-lookup"><span data-stu-id="0d6b6-583">*Showing the Geek Quiz web app with the image*</span></span>
2. <span data-ttu-id="0d6b6-584">キーを押して**F12**開発ツールを起動するには、選択、**ネットワーク**タブし、記録を開始します。</span><span class="sxs-lookup"><span data-stu-id="0d6b6-584">Press **F12** to launch the development tools, select the **Network** tab and start recording.</span></span>

    <span data-ttu-id="0d6b6-585">![ネットワークの記録を開始](maintainable-azure-websites-managing-change-and-scale/_static/image75.png "ネットワークの記録を開始")</span><span class="sxs-lookup"><span data-stu-id="0d6b6-585">![Starting network recording](maintainable-azure-websites-managing-change-and-scale/_static/image75.png "Starting network recording")</span></span>

    <span data-ttu-id="0d6b6-586">*ネットワークの記録を開始*</span><span class="sxs-lookup"><span data-stu-id="0d6b6-586">*Starting network recording*</span></span>
3. <span data-ttu-id="0d6b6-587">キーを押して**ctrl キーを押しながら f5 キーを押して**web ページを更新します。</span><span class="sxs-lookup"><span data-stu-id="0d6b6-587">Press **CTRL + F5** to refresh the web page.</span></span>
4. <span data-ttu-id="0d6b6-588">HTTP 要求が表示、ページの読み込みが完了すると、 **/img/logo-big.png** http URL **301**結果 (リダイレクト) と別の要求を`http://[YOUR-STORAGE-ACCOUNT].blob.core.windows.net/images/logo-big.png`URL と HTTP **200**結果。</span><span class="sxs-lookup"><span data-stu-id="0d6b6-588">Once the page has finished loading, you should see an HTTP request for the **/img/logo-big.png** URL with an HTTP **301** result (redirect) and another request for `http://[YOUR-STORAGE-ACCOUNT].blob.core.windows.net/images/logo-big.png` URL with a HTTP **200** result.</span></span>

    <span data-ttu-id="0d6b6-589">![URL リダイレクトの確認](maintainable-azure-websites-managing-change-and-scale/_static/image76.png "開発ツールでリダイレクトを示す")</span><span class="sxs-lookup"><span data-stu-id="0d6b6-589">![Verifying the URL redirect](maintainable-azure-websites-managing-change-and-scale/_static/image76.png "Showing the redirect in Dev Tools")</span></span>

    <span data-ttu-id="0d6b6-590">*URL リダイレクトの確認*</span><span class="sxs-lookup"><span data-stu-id="0d6b6-590">*Verifying the URL redirect*</span></span>

<a id="Exercise5"></a>
### <a name="exercise-5-using-autoscale-for-web-apps"></a><span data-ttu-id="0d6b6-591">手順 5:Web アプリの自動スケールの使用</span><span class="sxs-lookup"><span data-stu-id="0d6b6-591">Exercise 5: Using Autoscale for Web Apps</span></span>

> [!NOTE]
> <span data-ttu-id="0d6b6-592">Web ロードのサポートを必要があるために、この手順は省略可能で&amp;パフォーマンス テストで使用可能なだけ**Visual Studio 2013 Ultimate Edition**します。</span><span class="sxs-lookup"><span data-stu-id="0d6b6-592">This exercise is optional, since it requires support for Web Load &amp; Performance Testing which is only available for **Visual Studio 2013 Ultimate Edition**.</span></span> <span data-ttu-id="0d6b6-593">Visual Studio 2013 の特定の機能の詳細については、のバージョンを比較[ここ](https://www.microsoft.com/visualstudio/eng/products/compare)します。</span><span class="sxs-lookup"><span data-stu-id="0d6b6-593">For more information on specific Visual Studio 2013 features, compare versions [here](https://www.microsoft.com/visualstudio/eng/products/compare).</span></span>


<span data-ttu-id="0d6b6-594">**Azure App Service Web Apps**実行されている web アプリの自動スケール機能を提供して**標準モード**します。</span><span class="sxs-lookup"><span data-stu-id="0d6b6-594">**Azure App Service Web Apps** provides the Autoscale feature for web apps running in **Standard Mode**.</span></span> <span data-ttu-id="0d6b6-595">自動スケールは、Azure の負荷に応じて、web アプリのインスタンス数の自動スケールを使用できます。</span><span class="sxs-lookup"><span data-stu-id="0d6b6-595">Autoscale lets Azure automatically scale the instance count of your web app depending on the load.</span></span> <span data-ttu-id="0d6b6-596">自動スケールを有効にすると、Azure は、web アプリの CPU が 5 分ごとに 1 回チェックし、その時点で必要に応じてインスタンスが追加されます。</span><span class="sxs-lookup"><span data-stu-id="0d6b6-596">When Autoscale is enabled, Azure checks the CPU of your web app once every five minutes and adds instances as needed at that point in time.</span></span> <span data-ttu-id="0d6b6-597">CPU 使用率が低い場合インスタンスが削除されます 2 時間おきに web アプリのパフォーマンスが低下しないことを確認します。</span><span class="sxs-lookup"><span data-stu-id="0d6b6-597">If the CPU usage is low, Azure will remove instances once every two hours to ensure that the performance of your web app is not degraded.</span></span>

<span data-ttu-id="0d6b6-598">この演習では構成に必要な手順を参照してください、**自動スケール**の特徴として、**ギーク Quiz** web アプリ。</span><span class="sxs-lookup"><span data-stu-id="0d6b6-598">In this exercise you will go through the steps required to configure the **Autoscale** feature for the **Geek Quiz** web app.</span></span> <span data-ttu-id="0d6b6-599">インスタンスのアップグレードをトリガーするアプリケーションで十分な CPU の負荷を生成する Visual Studio ロード テストを実行して、この機能を確認します。</span><span class="sxs-lookup"><span data-stu-id="0d6b6-599">You will verify this feature by running a Visual Studio load test to generate enough CPU load on the application to trigger an instance upgrade.</span></span>

<a id="Ex5Task1"></a>
#### <a name="task-1--configuring-autoscale-based-on-the-cpu-metric"></a><span data-ttu-id="0d6b6-600">タスク 1 –、CPU のメトリックに基づく自動スケールを構成します。</span><span class="sxs-lookup"><span data-stu-id="0d6b6-600">Task 1 – Configuring Autoscale Based on the CPU Metric</span></span>

<span data-ttu-id="0d6b6-601">このタスクでは、手順 2 で作成した web アプリの自動スケール機能を有効にするのに、Azure 管理ポータルを使用します。</span><span class="sxs-lookup"><span data-stu-id="0d6b6-601">In this task you will use the Azure management portal to enable the Autoscale feature for the web app you created in Exercise 2.</span></span>

1. <span data-ttu-id="0d6b6-602">[Azure 管理ポータル](https://manage.windowsazure.com/)を選択します**Websites**手順 2 で作成した web アプリをクリックします。</span><span class="sxs-lookup"><span data-stu-id="0d6b6-602">In the [Azure management portal](https://manage.windowsazure.com/), select **Websites** and click the web app you created in Exercise 2.</span></span>
2. <span data-ttu-id="0d6b6-603">移動し、**スケール**ページ。</span><span class="sxs-lookup"><span data-stu-id="0d6b6-603">Navigate to the **Scale** page.</span></span> <span data-ttu-id="0d6b6-604">で、**容量**セクションで、 **CPU**の**メトリックによるスケール**構成します。</span><span class="sxs-lookup"><span data-stu-id="0d6b6-604">Under the **capacity** section, select **CPU** for the **Scale by Metric** configuration.</span></span>

    > [!NOTE]
    > <span data-ttu-id="0d6b6-605">CPU によってスケーリング、Azure は、CPU 使用率が変更された場合に、アプリが使用するインスタンスの数を動的に調整されます。</span><span class="sxs-lookup"><span data-stu-id="0d6b6-605">When scaling by CPU, Azure dynamically adjusts the number of instances that the app uses if the CPU usage changes.</span></span>

    <span data-ttu-id="0d6b6-606">![CPU によるスケールを選択する](maintainable-azure-websites-managing-change-and-scale/_static/image77.png "自動スケーリングの CPU のメトリックを選択します。")</span><span class="sxs-lookup"><span data-stu-id="0d6b6-606">![Selecting to scale by CPU](maintainable-azure-websites-managing-change-and-scale/_static/image77.png "Selecting the CPU metric for auto scaling")</span></span>

    <span data-ttu-id="0d6b6-607">*CPU によるスケールを選択します。*</span><span class="sxs-lookup"><span data-stu-id="0d6b6-607">*Selecting to scale by CPU*</span></span>
3. <span data-ttu-id="0d6b6-608">変更、**ターゲット CPU**構成**20**-**40** %。</span><span class="sxs-lookup"><span data-stu-id="0d6b6-608">Change the **Target CPU** configuration to **20**-**40** percent.</span></span>

    > [!NOTE]
    > <span data-ttu-id="0d6b6-609">この範囲は、web アプリの平均 CPU 使用率を表します。</span><span class="sxs-lookup"><span data-stu-id="0d6b6-609">This range represents the average CPU usage for your web app.</span></span> <span data-ttu-id="0d6b6-610">Azure では、追加または、この範囲内で web アプリを保持するインスタンスを削除します。</span><span class="sxs-lookup"><span data-stu-id="0d6b6-610">Azure will add or remove instances to keep your web app in this range.</span></span> <span data-ttu-id="0d6b6-611">拡張するために使用されるインスタンスの最小値と最大数がで指定された、**インスタンス数**構成します。</span><span class="sxs-lookup"><span data-stu-id="0d6b6-611">The minimum and maximum number of instances used for scaling is specified in the **Instance Count** configuration.</span></span> <span data-ttu-id="0d6b6-612">上またはその制限を超える azure になることはありません。</span><span class="sxs-lookup"><span data-stu-id="0d6b6-612">Azure will never go above or beyond that limit.</span></span>
    >
    > <span data-ttu-id="0d6b6-613">既定の**ターゲット CPU**値は、このラボの目的でのみ変更されます。</span><span class="sxs-lookup"><span data-stu-id="0d6b6-613">The default **Target CPU** values are modified just for the purposes of this lab.</span></span> <span data-ttu-id="0d6b6-614">中程度の負荷をアプリケーションに配置すると、CPU の範囲を小さい値に構成すると、自動スケールをトリガーする機会は増加します。</span><span class="sxs-lookup"><span data-stu-id="0d6b6-614">By configuring the CPU range with small values, you are increasing the chances to trigger Autoscale when a moderate load is placed on the application.</span></span>

    <span data-ttu-id="0d6b6-615">![CPU の 20 ~ 40% がターゲットの変更](maintainable-azure-websites-managing-change-and-scale/_static/image78.png "に 20 ~ 40% に CPU ターゲットの変更")</span><span class="sxs-lookup"><span data-stu-id="0d6b6-615">![Changing the target CPU to be between 20 and 40 percent](maintainable-azure-websites-managing-change-and-scale/_static/image78.png "Changing the target CPU to be between 20 and 40 percent")</span></span>

    <span data-ttu-id="0d6b6-616">*ターゲット CPU の 20 ~ 40% に変更します。*</span><span class="sxs-lookup"><span data-stu-id="0d6b6-616">*Changing the Target CPU to be between 20 and 40 percent*</span></span>
4. <span data-ttu-id="0d6b6-617">クリックして**保存**変更を保存するコマンド バーにします。</span><span class="sxs-lookup"><span data-stu-id="0d6b6-617">Click **Save** in the command bar to save the changes.</span></span>

<a id="Ex5Task2"></a>
#### <a name="task-2--load-testing-with-visual-studio"></a><span data-ttu-id="0d6b6-618">タスク 2 – Visual Studio によるロード テスト</span><span class="sxs-lookup"><span data-stu-id="0d6b6-618">Task 2 – Load Testing with Visual Studio</span></span>

<span data-ttu-id="0d6b6-619">できた**自動スケール**された作成は、構成されている、 **Web パフォーマンスとロード テストのプロジェクト**で Visual Studio で web アプリに対する CPU 負荷を生成します。</span><span class="sxs-lookup"><span data-stu-id="0d6b6-619">Now that **Autoscale** has been configured, you will create a **Web Performance and Load Test Project** in Visual Studio to generate some CPU load on your web app.</span></span>

1. <span data-ttu-id="0d6b6-620">開いている**Visual Studio Ultimate 2013**選択**ファイル |新しい |プロジェクトの追加.** 新しいソリューションを開始します。</span><span class="sxs-lookup"><span data-stu-id="0d6b6-620">Open **Visual Studio Ultimate 2013** and select **File | New | Project...** to start a new solution.</span></span>

    <span data-ttu-id="0d6b6-621">![新しいプロジェクトを作成する](maintainable-azure-websites-managing-change-and-scale/_static/image79.png "新しいプロジェクトを作成します。")</span><span class="sxs-lookup"><span data-stu-id="0d6b6-621">![Creating a new project](maintainable-azure-websites-managing-change-and-scale/_static/image79.png "Creating a new project")</span></span>

    <span data-ttu-id="0d6b6-622">*新しいプロジェクトを作成します。*</span><span class="sxs-lookup"><span data-stu-id="0d6b6-622">*Creating a new project*</span></span>
2. <span data-ttu-id="0d6b6-623">**新しいプロジェクト**ダイアログ ボックスで、 **Web パフォーマンスとロード テストのプロジェクト**下、 **(Visual C#) |テスト**タブ。確認します **.NET Framework 4.5**がプロジェクトに名前を選択すると、 *WebAndLoadTestProject*、選択、**場所** をクリック**OK**。</span><span class="sxs-lookup"><span data-stu-id="0d6b6-623">In the **New Project** dialog box, select **Web Performance and Load Test Project** under the **Visual C# | Test** tab. Make sure **.NET Framework 4.5** is selected, name the project *WebAndLoadTestProject*, choose a **Location** and click **OK**.</span></span>

    <span data-ttu-id="0d6b6-624">![新しい Web およびロード テスト プロジェクトを作成する](maintainable-azure-websites-managing-change-and-scale/_static/image80.png "Web およびロード テスト プロジェクトを新規作成")</span><span class="sxs-lookup"><span data-stu-id="0d6b6-624">![Creating a new Web and Load Test project](maintainable-azure-websites-managing-change-and-scale/_static/image80.png "Creating a new Web and Load Test project")</span></span>

    <span data-ttu-id="0d6b6-625">*新しい Web およびロード テスト プロジェクトを作成します。*</span><span class="sxs-lookup"><span data-stu-id="0d6b6-625">*Creating a new Web and Load Test project*</span></span>
3. <span data-ttu-id="0d6b6-626">**WebTest1.webtest**を右クリックし、 **WebTest1**ノードをクリックします**要求の追加**します。</span><span class="sxs-lookup"><span data-stu-id="0d6b6-626">In the **WebTest1.webtest** Right-click the **WebTest1** node and click **Add Request**.</span></span>

    <span data-ttu-id="0d6b6-627">![[Webtest1] への要求の追加](maintainable-azure-websites-managing-change-and-scale/_static/image81.png "[webtest1] への要求の追加")</span><span class="sxs-lookup"><span data-stu-id="0d6b6-627">![Adding a request to WebTest1](maintainable-azure-websites-managing-change-and-scale/_static/image81.png "Adding a request to WebTest1")</span></span>

    <span data-ttu-id="0d6b6-628">*[Webtest1] への要求の追加*</span><span class="sxs-lookup"><span data-stu-id="0d6b6-628">*Adding a request to WebTest1*</span></span>
4. <span data-ttu-id="0d6b6-629">**プロパティ**ウィンドウ、新しい要求のノードの更新、 **Url** web アプリの URL を指すプロパティ (例: *[ http://geek-quiz.azurewebsites.net/ ](http://geek-quiz.azurewebsites.net/)*).</span><span class="sxs-lookup"><span data-stu-id="0d6b6-629">In the **Properties** window of the new request node, update the **Url** property to point to the URL of your web app (e.g. *[http://geek-quiz.azurewebsites.net/](http://geek-quiz.azurewebsites.net/)*).</span></span>

    <span data-ttu-id="0d6b6-630">![Url プロパティを変更する](maintainable-azure-websites-managing-change-and-scale/_static/image82.png "Url プロパティを変更します。")</span><span class="sxs-lookup"><span data-stu-id="0d6b6-630">![Changing the Url property](maintainable-azure-websites-managing-change-and-scale/_static/image82.png "Changing the Url property")</span></span>

    <span data-ttu-id="0d6b6-631">*Url プロパティを変更します。*</span><span class="sxs-lookup"><span data-stu-id="0d6b6-631">*Changing the Url property*</span></span>
5. <span data-ttu-id="0d6b6-632">**WebTest1.webtest**ウィンドウで、右クリック**WebTest1**  をクリック**ループの追加.**.</span><span class="sxs-lookup"><span data-stu-id="0d6b6-632">In the **WebTest1.webtest** window, right-click **WebTest1** and click **Add Loop...**.</span></span>

    <span data-ttu-id="0d6b6-633">![[Webtest1] へのループの追加](maintainable-azure-websites-managing-change-and-scale/_static/image83.png "WebTest1 にループの追加")</span><span class="sxs-lookup"><span data-stu-id="0d6b6-633">![Adding a loop to WebTest1](maintainable-azure-websites-managing-change-and-scale/_static/image83.png "Adding a loop to WebTest1")</span></span>

    <span data-ttu-id="0d6b6-634">*[Webtest1] へのループの追加*</span><span class="sxs-lookup"><span data-stu-id="0d6b6-634">*Adding a loop to WebTest1*</span></span>
6. <span data-ttu-id="0d6b6-635">**条件付き規則の追加と項目をループ**ダイアログ ボックスで、 **For ループ**ルールし、次のプロパティを変更します。</span><span class="sxs-lookup"><span data-stu-id="0d6b6-635">In the **Add Conditional Rule and Items to Loop** dialog box, select the **For Loop** rule and modify the following properties.</span></span>

   1. <span data-ttu-id="0d6b6-636">**終了値。** 1000</span><span class="sxs-lookup"><span data-stu-id="0d6b6-636">**Terminating value:** 1000</span></span>
   2. <span data-ttu-id="0d6b6-637">**コンテキスト パラメーター名:** Iterator</span><span class="sxs-lookup"><span data-stu-id="0d6b6-637">**Context Parameter Name:** Iterator</span></span>
   3. <span data-ttu-id="0d6b6-638">**増分値。** 1</span><span class="sxs-lookup"><span data-stu-id="0d6b6-638">**Increment Value:** 1</span></span>

      <span data-ttu-id="0d6b6-639">![For ループのルールを選択し、プロパティを更新](maintainable-azure-websites-managing-change-and-scale/_static/image84.png "For ループのルールを選択し、プロパティの更新")</span><span class="sxs-lookup"><span data-stu-id="0d6b6-639">![Selecting the For Loop rule and updating the properties](maintainable-azure-websites-managing-change-and-scale/_static/image84.png "Selecting the For Loop rule and updating the properties")</span></span>

      <span data-ttu-id="0d6b6-640">*For ループのルールを選択し、プロパティの更新*</span><span class="sxs-lookup"><span data-stu-id="0d6b6-640">*Selecting the For Loop rule and updating the properties*</span></span>
7. <span data-ttu-id="0d6b6-641">で、**ループ内の項目**セクションで、ループの最初と最後の項目にする前に作成した要求を選択します。</span><span class="sxs-lookup"><span data-stu-id="0d6b6-641">Under the **Items in loop** section, select the request you created previously to be the first and last item for the loop.</span></span> <span data-ttu-id="0d6b6-642">**[OK]** をクリックして続行します。</span><span class="sxs-lookup"><span data-stu-id="0d6b6-642">Click **OK** to continue.</span></span>

    <span data-ttu-id="0d6b6-643">![ループの最初と最後の項目を選択する](maintainable-azure-websites-managing-change-and-scale/_static/image85.png "ループの最初と最後の項目を選択します。")</span><span class="sxs-lookup"><span data-stu-id="0d6b6-643">![Selecting the first and last items for the loop](maintainable-azure-websites-managing-change-and-scale/_static/image85.png "Selecting the first and last items for the loop")</span></span>

    <span data-ttu-id="0d6b6-644">*ループの最初と最後の項目を選択します。*</span><span class="sxs-lookup"><span data-stu-id="0d6b6-644">*Selecting the first and last items for the loop*</span></span>
8. <span data-ttu-id="0d6b6-645">**ソリューション エクスプ ローラー**を右クリックし、 **WebAndLoadTestProject**プロジェクトで、展開、**追加**メニュー選択し、**ロード テストしています.**.</span><span class="sxs-lookup"><span data-stu-id="0d6b6-645">In **Solution Explorer**, right-click the **WebAndLoadTestProject** project, expand the **Add** menu and select **Load Test...**.</span></span>

    <span data-ttu-id="0d6b6-646">![WebAndLoadTestProject プロジェクトへのロード テストの追加](maintainable-azure-websites-managing-change-and-scale/_static/image86.png "WebAndLoadTestProject プロジェクトへのロード テストの追加")</span><span class="sxs-lookup"><span data-stu-id="0d6b6-646">![Adding a Load Test to the WebAndLoadTestProject project](maintainable-azure-websites-managing-change-and-scale/_static/image86.png "Adding a Load Test to the WebAndLoadTestProject project")</span></span>

    <span data-ttu-id="0d6b6-647">*WebAndLoadTestProject プロジェクトへのロード テストの追加*</span><span class="sxs-lookup"><span data-stu-id="0d6b6-647">*Adding a Load Test to the WebAndLoadTestProject project*</span></span>
9. <span data-ttu-id="0d6b6-648">**新しいロード テスト ウィザード**ダイアログ ボックスで、をクリックして**次**します。</span><span class="sxs-lookup"><span data-stu-id="0d6b6-648">In the **New Load Test Wizard** dialog box, click **Next**.</span></span>

    <span data-ttu-id="0d6b6-649">![新しいロード テスト ウィザード](maintainable-azure-websites-managing-change-and-scale/_static/image87.png "新しいロード テスト ウィザード")</span><span class="sxs-lookup"><span data-stu-id="0d6b6-649">![New Load Test Wizard](maintainable-azure-websites-managing-change-and-scale/_static/image87.png "New Load Test Wizard")</span></span>

    <span data-ttu-id="0d6b6-650">*新しいロード テスト ウィザード*</span><span class="sxs-lookup"><span data-stu-id="0d6b6-650">*New Load Test Wizard*</span></span>
10. <span data-ttu-id="0d6b6-651">**シナリオ** ページで、**待ち時間を使用しないでください** をクリック**次**します。</span><span class="sxs-lookup"><span data-stu-id="0d6b6-651">In the **Scenario** page, select **Do not use think times** and click **Next**.</span></span>

    <span data-ttu-id="0d6b6-652">![待ち時間を使用しないように選択](maintainable-azure-websites-managing-change-and-scale/_static/image88.png "待ち時間を使用しないように選択します。")</span><span class="sxs-lookup"><span data-stu-id="0d6b6-652">![Selecting not to use think times](maintainable-azure-websites-managing-change-and-scale/_static/image88.png "Selecting not to use think times")</span></span>

    <span data-ttu-id="0d6b6-653">*待ち時間を使用しないように選択します。*</span><span class="sxs-lookup"><span data-stu-id="0d6b6-653">*Selecting not to use think times*</span></span>
11. <span data-ttu-id="0d6b6-654">**ロード パターン**ことを確認します ページで、**持続ロード**オプションを選択します。</span><span class="sxs-lookup"><span data-stu-id="0d6b6-654">In the **Load Pattern** page, make sure that the **Constant Load** option is selected.</span></span> <span data-ttu-id="0d6b6-655">変更、**ユーザー カウント**に設定**250**ユーザーとクリック **[次へ]** します。</span><span class="sxs-lookup"><span data-stu-id="0d6b6-655">Change the **User Count** setting to **250** users and click **Next**.</span></span>

    <span data-ttu-id="0d6b6-656">![ユーザー カウントを 250 に変更する](maintainable-azure-websites-managing-change-and-scale/_static/image89.png "を 250 ユーザー数を変更します。")</span><span class="sxs-lookup"><span data-stu-id="0d6b6-656">![Changing the user count to 250](maintainable-azure-websites-managing-change-and-scale/_static/image89.png "Changing the user count to 250")</span></span>

    <span data-ttu-id="0d6b6-657">*ユーザー カウントを 250 に変更します。*</span><span class="sxs-lookup"><span data-stu-id="0d6b6-657">*Changing the user count to 250*</span></span>
12. <span data-ttu-id="0d6b6-658">**テスト ミックス モデル** ページで、**時系列順に基づいて** をクリック**次**します。</span><span class="sxs-lookup"><span data-stu-id="0d6b6-658">In the **Test Mix Model** page, select **Based on sequential test order** and click **Next**.</span></span>

    <span data-ttu-id="0d6b6-659">![テスト ミックス モデルを選択する](maintainable-azure-websites-managing-change-and-scale/_static/image90.png "テスト ミックス モデルの選択")</span><span class="sxs-lookup"><span data-stu-id="0d6b6-659">![Selecting the test mix model](maintainable-azure-websites-managing-change-and-scale/_static/image90.png "Selecting the test mix model")</span></span>

    <span data-ttu-id="0d6b6-660">*テスト ミックス モデルの選択*</span><span class="sxs-lookup"><span data-stu-id="0d6b6-660">*Selecting the test mix model*</span></span>
13. <span data-ttu-id="0d6b6-661">**テスト ミックス モデル**] ページで [**追加しています.** テストをミックスに追加します。</span><span class="sxs-lookup"><span data-stu-id="0d6b6-661">In the **Test Mix Model** page, click **Add...** to add a test to the mix.</span></span>

    <span data-ttu-id="0d6b6-662">![テスト ミックスにテストを追加する](maintainable-azure-websites-managing-change-and-scale/_static/image91.png "テスト ミックスにテストを追加します。")</span><span class="sxs-lookup"><span data-stu-id="0d6b6-662">![Adding a test to the test mix](maintainable-azure-websites-managing-change-and-scale/_static/image91.png "Adding a test to the test mix")</span></span>

    <span data-ttu-id="0d6b6-663">*テスト ミックスにテストを追加します。*</span><span class="sxs-lookup"><span data-stu-id="0d6b6-663">*Adding a test to the test mix*</span></span>
14. <span data-ttu-id="0d6b6-664">**テストの追加**ダイアログ ボックスをダブルクリック**WebTest1**にテストを追加する、**選択されたテスト**一覧。</span><span class="sxs-lookup"><span data-stu-id="0d6b6-664">In the **Add Tests** dialog box, double-click **WebTest1** to add the test to the **Selected tests** list.</span></span> <span data-ttu-id="0d6b6-665">**[OK]** をクリックして続行します。</span><span class="sxs-lookup"><span data-stu-id="0d6b6-665">Click **OK** to continue.</span></span>

    <span data-ttu-id="0d6b6-666">![[Webtest1] テストを追加する](maintainable-azure-websites-managing-change-and-scale/_static/image92.png "[webtest1] テストを追加します。")</span><span class="sxs-lookup"><span data-stu-id="0d6b6-666">![Adding the WebTest1 test](maintainable-azure-websites-managing-change-and-scale/_static/image92.png "Adding the WebTest1 test")</span></span>

    <span data-ttu-id="0d6b6-667">*[Webtest1] テストを追加します。*</span><span class="sxs-lookup"><span data-stu-id="0d6b6-667">*Adding the WebTest1 test*</span></span>
15. <span data-ttu-id="0d6b6-668">戻り、**テスト ミックス**] ページで [**次**します。</span><span class="sxs-lookup"><span data-stu-id="0d6b6-668">Back in the **Test Mix** page, click **Next**.</span></span>

    <span data-ttu-id="0d6b6-669">![テスト ミックスのページを完了](maintainable-azure-websites-managing-change-and-scale/_static/image93.png "テスト ミックスのページの完了")</span><span class="sxs-lookup"><span data-stu-id="0d6b6-669">![Completing the Test Mix page](maintainable-azure-websites-managing-change-and-scale/_static/image93.png "Completing the Test Mix page")</span></span>

    <span data-ttu-id="0d6b6-670">*テスト ミックスのページの完了*</span><span class="sxs-lookup"><span data-stu-id="0d6b6-670">*Completing the Test Mix page*</span></span>
16. <span data-ttu-id="0d6b6-671">**ネットワーク ミックス**] ページで [**次**します。</span><span class="sxs-lookup"><span data-stu-id="0d6b6-671">In the **Network Mix** page, click **Next**.</span></span>

    <span data-ttu-id="0d6b6-672">![クリックして、[ネットワーク ミックス] ページで次に](maintainable-azure-websites-managing-change-and-scale/_static/image94.png "ネットワーク ミックス ページで次をクリックすると")</span><span class="sxs-lookup"><span data-stu-id="0d6b6-672">![Clicking next in the Network Mix page](maintainable-azure-websites-managing-change-and-scale/_static/image94.png "Clicking next in the Network Mix page")</span></span>

    <span data-ttu-id="0d6b6-673">*ネットワーク ミックス ページで 次へ をクリックします。*</span><span class="sxs-lookup"><span data-stu-id="0d6b6-673">*Clicking next in the Network Mix page*</span></span>
17. <span data-ttu-id="0d6b6-674">**ブラウザー ミックス**] ページで、[ **Internet Explorer 10.0**ブラウザーの種類をクリックします**次**します。</span><span class="sxs-lookup"><span data-stu-id="0d6b6-674">In the **Browser Mix** page, select **Internet Explorer 10.0** as the browser type and click **Next**.</span></span>

    <span data-ttu-id="0d6b6-675">![ブラウザーの種類を選択する](maintainable-azure-websites-managing-change-and-scale/_static/image95.png "ブラウザーの種類を選択します。")</span><span class="sxs-lookup"><span data-stu-id="0d6b6-675">![Selecting the browser type](maintainable-azure-websites-managing-change-and-scale/_static/image95.png "Selecting the browser type")</span></span>

    <span data-ttu-id="0d6b6-676">*ブラウザーの種類を選択します。*</span><span class="sxs-lookup"><span data-stu-id="0d6b6-676">*Selecting the browser type*</span></span>
18. <span data-ttu-id="0d6b6-677">**カウンター セット**] ページで [**次**します。</span><span class="sxs-lookup"><span data-stu-id="0d6b6-677">In the **Counter Sets** page, click **Next**.</span></span>

    <span data-ttu-id="0d6b6-678">![カウンター セット ページで 次へ をクリックすると](maintainable-azure-websites-managing-change-and-scale/_static/image96.png "カウンター セット ページでは、次をクリックすると")</span><span class="sxs-lookup"><span data-stu-id="0d6b6-678">![Clicking Next in the Counter Sets page](maintainable-azure-websites-managing-change-and-scale/_static/image96.png "Clicking Next in the Counter Sets page")</span></span>

    <span data-ttu-id="0d6b6-679">*カウンター セット ページで 次へ をクリックして*</span><span class="sxs-lookup"><span data-stu-id="0d6b6-679">*Clicking Next in the Counter Sets page*</span></span>
19. <span data-ttu-id="0d6b6-680">**実行設定** ページで、設定、**ロード テストの継続時間**に**5 分** をクリック**完了**します。</span><span class="sxs-lookup"><span data-stu-id="0d6b6-680">In the **Run Settings** page, set the **Load test duration** to **5 minutes** and click **Finish**.</span></span>

    <span data-ttu-id="0d6b6-681">![ロード テストの継続時間を 5 分に設定](maintainable-azure-websites-managing-change-and-scale/_static/image97.png "ロード テストの継続時間を 5 分に設定します。")</span><span class="sxs-lookup"><span data-stu-id="0d6b6-681">![Setting the load test duration to 5 minutes](maintainable-azure-websites-managing-change-and-scale/_static/image97.png "Setting the load test duration to 5 minutes")</span></span>

    <span data-ttu-id="0d6b6-682">*ロード テストの継続時間を 5 分に設定します。*</span><span class="sxs-lookup"><span data-stu-id="0d6b6-682">*Setting the load test duration to 5 minutes*</span></span>
20. <span data-ttu-id="0d6b6-683">**ソリューション エクスプ ローラー**、ダブルクリックして、 **Local.settings**テストの設定を表示するファイル。</span><span class="sxs-lookup"><span data-stu-id="0d6b6-683">In **Solution Explorer**, double-click the **Local.settings** file to explore the test settings.</span></span> <span data-ttu-id="0d6b6-684">既定では、Visual Studio は、テストを実行するのにローカル コンピューターを使用します。</span><span class="sxs-lookup"><span data-stu-id="0d6b6-684">By default, Visual Studio uses your local computer to run the tests.</span></span>

    > [!NOTE]
    > <span data-ttu-id="0d6b6-685">使用して、クラウドでロード テストを実行するテスト プロジェクトを構成する代わりに、 **Azure テスト計画**します。</span><span class="sxs-lookup"><span data-stu-id="0d6b6-685">Alternatively, you can configure your test project to run the load tests in the cloud using **Azure Test Plans**.</span></span> <span data-ttu-id="0d6b6-686">Azure のテスト計画には、クラウド ベースのロード テスト サービスをより現実的な負荷をシミュレートする、CPU 容量、使用可能なメモリ、ネットワーク帯域幅などのローカル環境の制約を回避が提供します。</span><span class="sxs-lookup"><span data-stu-id="0d6b6-686">Azure Test Plans provides a cloud-based load testing service that simulates a more realistic load, avoiding local environment constraints like CPU capacity, available memory, and network bandwidth.</span></span> <span data-ttu-id="0d6b6-687">Azure のテスト計画を使用して、ロード テストを実行する方法の詳細については、次を参照してください。[ロード テスト シナリオ](/azure/devops/test/load-test/overview?view=vsts)します。</span><span class="sxs-lookup"><span data-stu-id="0d6b6-687">For more information about using Azure Test Plans to run load tests, see [Load testing scenarios](/azure/devops/test/load-test/overview?view=vsts).</span></span>

    ![テストの設定](maintainable-azure-websites-managing-change-and-scale/_static/image98.png)

<a id="Ex5Task3"></a>
#### <a name="task-3--autoscale-verification"></a><span data-ttu-id="0d6b6-689">タスク 3 – 自動スケール検証</span><span class="sxs-lookup"><span data-stu-id="0d6b6-689">Task 3 – Autoscale Verification</span></span>

<span data-ttu-id="0d6b6-690">前のタスクで作成したロード テストを実行するようになりましたし負荷の下で web アプリの動作を参照してください。</span><span class="sxs-lookup"><span data-stu-id="0d6b6-690">You will now execute the load test you created in the previous task and see how your web app behaves under load.</span></span>

1. <span data-ttu-id="0d6b6-691">**ソリューション エクスプ ローラー**、ダブルクリックして**LoadTest1.loadtest**をロード テストを開きます。</span><span class="sxs-lookup"><span data-stu-id="0d6b6-691">In **Solution Explorer**, double-click **LoadTest1.loadtest** to open the load test.</span></span>

    <span data-ttu-id="0d6b6-692">![LoadTest1.loadtest を開く](maintainable-azure-websites-managing-change-and-scale/_static/image99.png "LoadTest1.loadtest を開く")</span><span class="sxs-lookup"><span data-stu-id="0d6b6-692">![Opening LoadTest1.loadtest](maintainable-azure-websites-managing-change-and-scale/_static/image99.png "Opening LoadTest1.loadtest")</span></span>

    <span data-ttu-id="0d6b6-693">*LoadTest1.loadtest を開く*</span><span class="sxs-lookup"><span data-stu-id="0d6b6-693">*Opening LoadTest1.loadtest*</span></span>
2. <span data-ttu-id="0d6b6-694">**LoadTest1.loadtest**ウィンドウで、ロード テストを実行するツールボックス内の最初のボタンをクリックします。</span><span class="sxs-lookup"><span data-stu-id="0d6b6-694">In the **LoadTest1.loadtest** window, click the first button in the toolbox to run the load test.</span></span>

    <span data-ttu-id="0d6b6-695">![ロード テストを実行している](maintainable-azure-websites-managing-change-and-scale/_static/image100.png "ロード テストの実行")</span><span class="sxs-lookup"><span data-stu-id="0d6b6-695">![Running the load test](maintainable-azure-websites-managing-change-and-scale/_static/image100.png "Running the load test")</span></span>

    <span data-ttu-id="0d6b6-696">*ロード テストの実行*</span><span class="sxs-lookup"><span data-stu-id="0d6b6-696">*Running the load test*</span></span>
3. <span data-ttu-id="0d6b6-697">ロード テストが完了するまで待機します。</span><span class="sxs-lookup"><span data-stu-id="0d6b6-697">Wait until the load test completes.</span></span>

    > [!NOTE]
    > <span data-ttu-id="0d6b6-698">ロード テストでは、web アプリを同時に要求を送信する複数のユーザーをシミュレートします。</span><span class="sxs-lookup"><span data-stu-id="0d6b6-698">The load test simulates multiple users that send requests to the web app simultaneously.</span></span> <span data-ttu-id="0d6b6-699">テストが実行されている場合は、すべてのエラー、警告、またはロード テストの実行に関連するその他の情報を検出するために使用可能なカウンターを監視できます。</span><span class="sxs-lookup"><span data-stu-id="0d6b6-699">When the test is running, you can monitor the available counters to detect any errors, warnings or other information related to your load test run.</span></span>

    <span data-ttu-id="0d6b6-700">![ロード テストを実行している](maintainable-azure-websites-managing-change-and-scale/_static/image101.png "ロード テストが完了するまで待機しています")</span><span class="sxs-lookup"><span data-stu-id="0d6b6-700">![Load test running](maintainable-azure-websites-managing-change-and-scale/_static/image101.png "Waiting until the load test completes")</span></span>

    <span data-ttu-id="0d6b6-701">*ロード テストの実行*</span><span class="sxs-lookup"><span data-stu-id="0d6b6-701">*Load test running*</span></span>
4. <span data-ttu-id="0d6b6-702">テストが完了すると、管理ポータルに戻るしに移動します、**スケール**web アプリのページ。</span><span class="sxs-lookup"><span data-stu-id="0d6b6-702">Once the test completes, go back to the management portal and navigate to the **Scale** page of your web app.</span></span> <span data-ttu-id="0d6b6-703">で、**容量**セクションに表示されます、グラフの新しいインスタンスが自動的にデプロイされたことです。</span><span class="sxs-lookup"><span data-stu-id="0d6b6-703">Under the **capacity** section, you should see in the graph that a new instance was automatically deployed.</span></span>

    ![新しいインスタンスを自動的にデプロイ](maintainable-azure-websites-managing-change-and-scale/_static/image102.png)

    <span data-ttu-id="0d6b6-705">*新しいインスタンスを自動的にデプロイ*</span><span class="sxs-lookup"><span data-stu-id="0d6b6-705">*New instance automatically deployed*</span></span>

    > [!NOTE]
    > <span data-ttu-id="0d6b6-706">変更、グラフに表示されるまで数分かかる場合があります (キーを押して**ctrl キーを押しながら f5 キーを押して**ページを更新するには、定期的に)。</span><span class="sxs-lookup"><span data-stu-id="0d6b6-706">It may take several minutes for the changes to appear in the graph (press **CTRL + F5** periodically to refresh the page).</span></span> <span data-ttu-id="0d6b6-707">すべての変更が表示されない場合、次の操作を行うことができます。</span><span class="sxs-lookup"><span data-stu-id="0d6b6-707">If you do not see any changes, you can try the following:</span></span>
    >
    > - <span data-ttu-id="0d6b6-708">ロード テストの期間は長くなります (例: に**10 分**)</span><span class="sxs-lookup"><span data-stu-id="0d6b6-708">Increase the duration of the load test (e.g. to **10 minutes**)</span></span>
    > - <span data-ttu-id="0d6b6-709">最大値と最小値を減らして、**ターゲット CPU**範囲は、web アプリの自動スケールの構成</span><span class="sxs-lookup"><span data-stu-id="0d6b6-709">Reduce the maximum and minimum values of the **Target CPU** range in the Autoscale configuration of your web app</span></span>
    > - <span data-ttu-id="0d6b6-710">使用してクラウドでロード テストの実行**Azure テスト計画**します。</span><span class="sxs-lookup"><span data-stu-id="0d6b6-710">Run the load test in the cloud with **Azure Test Plans**.</span></span> <span data-ttu-id="0d6b6-711">詳細については[ここ](/azure/devops/test/load-test/index?view=vsts)</span><span class="sxs-lookup"><span data-stu-id="0d6b6-711">More information [here](/azure/devops/test/load-test/index?view=vsts)</span></span>

---

<a id="Summary"></a>
## <a name="summary"></a><span data-ttu-id="0d6b6-712">まとめ</span><span class="sxs-lookup"><span data-stu-id="0d6b6-712">Summary</span></span>

<span data-ttu-id="0d6b6-713">このハンズオン ラボを設定して、Azure で web アプリを運用環境にアプリケーションを配置する方法について説明しました。</span><span class="sxs-lookup"><span data-stu-id="0d6b6-713">In this hands-on lab, you learned how to set up and deploy your application to production web apps in Azure.</span></span> <span data-ttu-id="0d6b6-714">使用してデータベースの更新を検出して開始した**Entity Framework Code First Migrations**、しを使用して、サイトの新しいバージョンを展開することで続き**Git**へのロールバックを実行して、サイトの最新の安定バージョン。</span><span class="sxs-lookup"><span data-stu-id="0d6b6-714">You started by detecting and updating your databases using **Entity Framework Code First Migrations**, then continued by deploying new versions of your site using **Git** and performing rollbacks to the latest stable version of your site.</span></span> <span data-ttu-id="0d6b6-715">さらに、記憶域を使用して静的コンテンツを Blob コンテナーに移動するアプリをスケールする方法を学習しました。</span><span class="sxs-lookup"><span data-stu-id="0d6b6-715">Additionally, you learned how to scale your app using Storage to move your static content to a Blob container.</span></span>
